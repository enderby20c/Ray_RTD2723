// RTD2020UserFwFileNameStringTable.h
// First Byte shall be the number of File Name char (File Extension included). A valid number of File Name should be 26 or less.
// Second Byte shall be 1 if there's any ' ' (SPACE) exist in the file name, and shall be 0 if there's not.
// For Example : RTK_MM_PROGRAM.bin (number of char is 18, and no SPACE exist)
    18,  0,
    'R', 'T', 'K',
    '_', 'M', 'M',  '_',
    'P', 'R', 'O',  'G',
    'R', 'A', 'M',  '.',
    'b', 'i', 'n',