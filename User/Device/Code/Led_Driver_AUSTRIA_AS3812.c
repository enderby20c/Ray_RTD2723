...
//----------------------------------------------------------------------------------------------------
// ID Code      : Led_Driver_AUSTRIA_AS3812.c
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
#if(_DEVICE_LED_DRIVER_TYPE == _DEVICE_LED_DRIVER_03_AUSTRIA_AS3812)

//****************************************************************************
// DEFINITIONS / MACROS
//****************************************************************************
#if(_LOCAL_DIMMING_HDR10_BOOST_SUPPORT == _ON)
#define _DEVICE_LED_DRIVER_DEFALUIT_DAC             0x168
#else
#define _DEVICE_LED_DRIVER_DEFALUIT_DAC             0x2D0
#endif
...
// Since AS3812 Max HSYNC Freq. is 20MHz
...
{
    EnumDeviceLedDriver enumDriverIndex = 0;
    BYTE pucTemp[4];
    ...
    pucTemp[0] = 0xFF;  // CH1~CH8 current use ILEDrange = "1", 900nits
    pucTemp[1] = 0xFF;  // CH9~CH16 current use ILEDrange = "1", 900nits
    UserCommonColorSpiSetCommandRegion(_DEVICE_LED_DRIVER_ALL, 0x64, 2, pucTemp, _FALSE);

    for(enumDriverIndex = _DEVICE_LED_DRIVER_001; enumDriverIndex <= _DEVICE_LED_DRIVER_032; enumDriverIndex++)
    {
        if(enumDriverIndex < _DEVICE_LED_DRIVER_017)
        {
            pucTemp[0] = 0x00;
            pucTemp[1] = 0x00;
        }
        ...
        UserCommonColorSpiSetCommandRegion(enumDriverIndex, 0x05, 2, pucTemp, _FALSE);
    }
...
#endif
#endif
#endif // End of #if(_DEVICE_LED_DRIVER_TYPE == _DEVICE_LED_DRIVER_02_AUSTRIA_AS3812)
