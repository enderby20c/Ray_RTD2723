#################################
# build script for 32-bit kernel CPU(KCPU)
#################################
...
test_condition_check(${PROJECT_NAME})
...
test_condition(
    ${PROJECT_NAME}
    "_HW_CPU32_KCPU_MEMORYMAP_ACCESS_DDR == _ON"
    KCPU_ACCESS_DDR
)
...
if( NOT NO_LINK )
    ...
    set(PROJECT_LINK_FILE
        "${OUTPUT_DIR}/COMMON_FILES/${PROJECT_NAME}.ld"
    )
    set(PROJECT_MAP_FILE
        "${OUTPUT_DIR}/COMMON_FILES/${PROJECT_NAME}.map"
    )
...
add_custom_target("pre_build"
        ./build_actions_KCPU_firmware.sh pre_build
    DEPENDS
...
if( NOT NO_LINK )
    add_custom_command(TARGET ${PROJECT_NAME}
        PRE_LINK
            COMMAND
                ./build_actions_KCPU_firmware.sh pre_link
            COMMENT
                "run pre-link"
    )
    ...
    add_custom_command(TARGET ${PROJECT_NAME}
        POST_BUILD
            COMMAND
                ./build_actions_KCPU_firmware.sh post_build
            COMMENT
                "run post-build"
    )
endif()
...
add_custom_target("clean_all"
        ./build_actions_KCPU_firmware.sh clean_all
    COMMENT
        "run clean"
)
...
add_custom_command(
    OUTPUT
        "${CMAKE_OUTPUT_DIR}/gcc_cmd.sh"
    COMMAND
        echo ${CMAKE_C_COMPILER} ${C_FLAGS} "$<JOIN:$<TARGET_PROPERTY:${PROJECT_NAME},COMPILE_OPTIONS>, >" "-D$<JOIN:$<TARGET_PROPERTY:${PROJECT_NAME},COMPILE_DEFINITIONS>, -D>" "-I\"$<JOIN:$<TARGET_PROPERTY:${PROJECT_NAME},INCLUDE_DIRECTORIES>,\" -I\">\"" "\"\$\$\"*" > "${CMAKE_OUTPUT_DIR}/gcc_cmd.sh"
    COMMAND
...
add_custom_command(
    OUTPUT
        "${CMAKE_OUTPUT_DIR}/cpp_cmd.sh"
    COMMAND
        echo ${CPP} "-D$<JOIN:$<TARGET_PROPERTY:${PROJECT_NAME},COMPILE_DEFINITIONS>, -D>" "-I\"$<JOIN:$<TARGET_PROPERTY:${PROJECT_NAME},INCLUDE_DIRECTORIES>,\" -I\">\"" "\"\$\$\"*" > "${CMAKE_OUTPUT_DIR}/cpp_cmd.sh"
    COMMAND
...
#################################
set(TEMPLATE_FILES
    "build_actions/build_actions_KCPU_firmware.sh"
    ...
    "build_options_rename_off_region_data"
)
