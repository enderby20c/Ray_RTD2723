...
#include "ModeSearch/UserCommonModeSearch.h"
...
//****************************************************************************
BYTE g_ucModeSearchCtrl;
WORD g_usModeSearchCurrGTFValue;
...
BYTE UserCommonModeSearchModeVGA(void)
{
    ...
    if(ucModeIndex < _MAX_PRESET_MODE)
    {
        ...
        ucModeIndex = UserInterfaceVgaGetConfuseMode(ucModeIndex);
...
bit UserCommonModeSearchHVSyncPulseCheck(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pucModeBestInx)
{
    ...
    while(tHSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].ucFirstMode != _MODE_NOSUPPORT)
    {
        ...
        {
            if(GET_INPUT_TIMING_HSYNCWIDTH() <= (WORD)((DWORD)tHSYNC_PULSE_COUNT_MODE_TABLE[ucCnt].usMidSyncPulseCount * ScalerVgaSyncProcGetCurrentClockFreq() / 27000))
            {
...
bit UserCommonModeSearchCompareByVideoModeFlg(BYTE ucLastModeCnt, BYTE ucCurrentModeCnt, BYTE *pucModeBestInx)
{
    ...
    {
        if(ScalerVgaSyncProcGetSyncPad() == _TRUE)
        {
            *pucModeBestInx = ucLastModeCnt;
        }
        else
...
void UserCommonModeSearchUpdateInputDataRefFIFO(BYTE ucFifoIndex, BYTE ucIndex)
{
    ...
    if((UserCommonModeSearchCheckFIFOSetting(g_stVgaModeUserData, ucIndex) == _FALSE) || (UserInterfaceVgaGetLoadModeTableDefaultStatus() == _LOAD_MODE_TABLE_TRUE))
    {
...
// Input Value  : ucDataType --> Data Type in StructMeasureTimingInfo
