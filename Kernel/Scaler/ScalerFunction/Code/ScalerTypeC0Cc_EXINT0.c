...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerTypeC0Cc_EXINT0.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcIntHandler_EXINT0(void)
{
    ...
    {   // (cc1_vconn_shutdown_irq == 1) && (cc1_vconn_shutdown_irq_en = _ENABLE)
        ...
        ScalerSetBit_EXINT(P66_71_CC1_VCONN_OCP_INT, ~(_BIT5 | _BIT2 | _BIT1 | _BIT0), _BIT1);
        ...
        if(GET_TYPE_C_0_CC_CONNECT_EN() == _TRUE)
        {
            ...
            ScalerTypeC0PdSetSystemEvent_EXINT0(_PD_SYS_EVENT_VCONN_OCP);
        }
    }
    ...
    {   // (cc2_vconn_shutdown_irq == 1) && (cc2_vconn_shutdown_irq_en == _ENABLE)
        ...
        ScalerSetBit_EXINT(P66_72_CC2_VCONN_OCP_INT, ~(_BIT5 | _BIT2 | _BIT1 | _BIT0), _BIT1);
    ...
    if(ScalerGetBit_EXINT(P66_30_CC_HW_MODE_CTRL, (_BIT3 | _BIT2)) == (_BIT3 | _BIT2))
    {
        ...
        // [0x6630] : [3] hw_done_irq, [2] hw_done_irq_en
        // Disable and Clear hw_done_irq
        ScalerSetBit_EXINT(P66_30_CC_HW_MODE_CTRL, ~(_BIT3 | _BIT2 | _BIT1), _BIT3);

        if(GET_TYPE_C_0_CC_CONNECT_EN() == _TRUE)
        {
            // Set CC to Initial Setting (Orientation / RA_EXIST / Turn on Vconn & Vbus, SNK Unattach Vth to 3.67V)
            if(ScalerTypeC0CcAttachedSetting_EXINT0(&enumPdFwState) == _TRUE)
            {
                // [0x6630] : [3] hw_done_irq, [2] hw_done_irq_en, [1] hw_unattach_irq, [0] hw_unattach_irq_en
                if((GET_TYPE_C_0_CC_PORT_ROLE() == _TYPE_C_PORT_DRP) && (GET_TYPE_C_0_CC_DRP_PREFER() == _TYPE_C_DRP_TRY_SRC) && (GET_TYPE_C_0_CC_HW_STATE() == _TYPE_C_CC_HW_FSM_ATTACHED_SRC))
                {
                    // DRP Try Src FW Solution : Disable hw_done_irq and hw_unattach_irq
                    ScalerSetBit_EXINT(P66_30_CC_HW_MODE_CTRL, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT1));
                ...
                if(GET_TYPE_C_0_PD_FUNCTION_EN() == _TRUE)
                {
                    ...
                    ScalerTypeC0PdAttachedSetting_EXINT0(enumPdFwState);

                    // BMC Related Setting
                    ScalerTypeC0CcSetBMC_EXINT0(_ENABLE);
                }

                // Turn On U3 & Hub Function
#if(_EMB_TYPE_C_0_PD_BEHAVIOR_TYPE == _TYPE_C_DP_RX)
                ScalerDpPhyRxSetAltModeLaneMapping_EXINT0(enumInputPort, _TYPE_C_PIN_ASSIGNMENT_NONE, GET_TYPE_C_0_CC_ORIENTATION());
#elif(_EMB_TYPE_C_0_PD_BEHAVIOR_TYPE == _TYPE_C_DP_TX)
                ScalerDpPhyTxSetAltModeLaneMapping_EXINT0(enumOutputPort, _TYPE_C_PIN_ASSIGNMENT_NONE, GET_TYPE_C_0_CC_ORIENTATION());
#endif  // End of #if(_EMB_TYPE_C_0_PD_BEHAVIOR_TYPE == _TYPE_C_DP_RX)

                SET_TYPE_C_0_CC_ATTACHED();
                SET_TYPE_C_0_CC_FW_STATE(_TYPE_C_CC_EMB_FW_FSM_ATTACHED);
            }
            else
            {
    ...
    {   // (hw_unattach_irq == 1) && (hw_unattach_irq_en == Enable)
        ...
        if(GET_TYPE_C_0_CC_CONNECT_EN() == _TRUE)
        {
            ...
            ScalerTypeC0CcUnattachFlow_EXINT0();
        }
    }
    ...
    {   // ((cc1_r_chg_irq == 1) && (cc1_r_chg_irq_en == Enable)) || ((cc2_r_chg_irq == 1) && (cc2_r_chg_irq_en == Enable))
        ...
        ScalerSetBit_EXINT(P66_2C_CC_R_CHG_INT, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4 | _BIT0), (_BIT7 | _BIT5));
    ...
    if(ScalerGetBit_EXINT(P66_58_DBCC_SNK_DET_COND, (_BIT1 | _BIT0)) == (_BIT1 | _BIT0))
    {
        ...
        ScalerSetBit_EXINT(P66_58_DBCC_SNK_DET_COND, ~(_BIT3 | _BIT1 | _BIT0), _BIT1);
        ...
        if(GET_TYPE_C_0_CC_CONNECT_EN() == _TRUE)
        {
            ...
            SET_TYPE_C_0_CC_TRY_WAIT_SNK_STATE();

            // Do Unattach Flow When HW FSM Transition from Attached.SRC to TryWait.SNK
            if(GET_TYPE_C_0_CC_ATTACHED_SRC_TO_TRY_WAIT_SNK() == _TRUE)
            {
                ...
                // Disable BMC Channel
                ScalerTypeC0CcSetBMC_EXINT0(_DISABLE);
                ...
                ScalerTimerWDActivateTimerEvent_EXINT0(_UNATTACH_VBUS_DISCHARGE_TIMER, _SCALER_WD_TIMER_EVENT_CC_0_CHECK_VBUS_0V);
#endif

#if(_TYPE_C_EIZO_CUSTOMIZED_PMIC_SIC437A_SUPPORT == _ON)
                // [Eizo] SIC437A Customized Power Control : SRC/SNK Unattach
                ScalerTypeC0PmicSic437aUnattachProc_EXINT0(ScalerTypeC0PdGetPowerRole_EXINT0());
#endif
                // Reset PD
                ScalerTypeC0PdReset_EXINT0();
                ...
                ScalerSetBit_EXINT(P66_0C_TPC_CC_CTRL, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), (_BIT7 | _BIT6 | _BIT5 | _BIT4));
            }

            // [0x665A/5B] Setting FW Mode Rd Resistor
            ScalerSetByte_EXINT(P66_5A_CC_FW_CC1_Z_CFG, _BIT7);
            ScalerSetByte_EXINT(P66_5B_CC_FW_CC2_Z_CFG, _BIT7);

            // [0x6630] Change to FW Mode to Detect Attach or Unattach
            // [7] tpc_cc_mode_sel = 0
            ScalerSetBit_EXINT(P66_30_CC_HW_MODE_CTRL, ~(_BIT7 | _BIT3 | _BIT1), 0x00);

            // Set Power Role for Detecting CC1/CC2 Resistor = SNK
            ScalerSetBit_EXINT(P66_2B_CC_DIG_DET_R, ~_BIT7, _BIT7);

            // Set Vth2/Vth3 to 660mV/1230mV
            ScalerSetByte_EXINT(P66_28_VTH_SEL, 0x00);

            // Set Double CC as SNK Detect Open-Open Enable Bit
            ScalerSetBit_EXINT(P66_58_DBCC_SNK_DET_COND, ~(_BIT4 | _BIT3 | _BIT1), _BIT4);

            // Set De-Bounce Threshold For Open-Open ([1:0] tPDDebounce = b'01)
            ScalerSetBit_EXINT(P66_55_DBCC_SNK_DB_SEL, ~(_BIT1 | _BIT0), _BIT0);

            // Set Double CC De-glitch
            ScalerSetBit_EXINT(P66_50_DBCC_DGH_EN, ~_BIT0, _BIT0);
            ScalerSetByte_EXINT(P66_51_DBCC_DGH_TH, ScalerGetByte_EXINT(P66_05_CC1_DGH_LV));

            // [0x6641/42] Clear & Enable dbcc_det_irq
            ScalerSetBit_EXINT(P66_41_CC_FW_DET_INT, ~(_BIT7 | _BIT5 | _BIT4 | _BIT3 | _BIT2), _BIT7);
            ScalerSetBit_EXINT(P66_42_CC_FW_DET_INT_EN, ~_BIT7, _BIT7);

            // Unattach Condition : CC = Open + Open for tPDDebounce
            // Enable Both CC Detect Equal Condition
            ScalerSetBit_EXINT(P66_40_CC_FW_DET_CTRL, ~_BIT7, _BIT7);

            // Attach Condition : After tCCDebounce & Vbus Detected
            // Active WD Timer Event To Detect Vbus 5V After tCCDebounce
            ScalerTimerWDActivateTimerEvent_EXINT0(110, _SCALER_WD_TIMER_EVENT_CC_0_DETECT_VBUS);
        }
    }

    if((ScalerGetBit_EXINT(P66_41_CC_FW_DET_INT, _BIT7) == _BIT7) && (ScalerGetBit_EXINT(P66_42_CC_FW_DET_INT_EN, _BIT7) == _BIT7))
    {
        // DRP Try Src FW Solution : When Both CC Detect Open-Open for tPDDebounce, Set HW State to Unattached.SNK
        // [0x6641/42] Clear & Disable dbcc_det_irq
        ScalerSetBit_EXINT(P66_41_CC_FW_DET_INT, ~(_BIT7 | _BIT5 | _BIT4 | _BIT3 | _BIT2), _BIT7);
        ScalerSetBit_EXINT(P66_42_CC_FW_DET_INT_EN, ~_BIT7, 0x00);

        if(GET_TYPE_C_0_CC_CONNECT_EN() == _TRUE)
        {
            // Cancel WDTimerEvent
            ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_CC_0_DETECT_VBUS);

            // Disable [adc0_comp_vbus_on_en]
            ...
            // [0x6635] Set HW State to Unattached.SNK
            ScalerSetBit_EXINT(P66_30_CC_HW_MODE_CTRL, ~(_BIT6 | _BIT3 | _BIT1), 0x00);
            ScalerSetBit_EXINT(P66_35_CC_HW_STATE_TRANSIT, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), _BIT4);

            // [0x6630] : [7] tpc_cc_mode_sel, [6] hw_mode_fsm_rst, [3] hw_done_irq, [1] hw_unattach_irq
            // Change Back to HW Mode, Clear (hw_done_irq) & (hw_unattach_irq)
            ScalerSetBit_EXINT(P66_30_CC_HW_MODE_CTRL, ~(_BIT7 | _BIT6 | _BIT3 | _BIT1), (_BIT7 | _BIT6 | _BIT3 | _BIT1));
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcSnkDetRpCtrl_EXINT0(bit bAction)
{
    ...
    if(bAction == _ENABLE)
    {
        ...
        ScalerSetBit_EXINT(P66_37_CC_HW_Z_CFG, ~_BIT0, 0x00);
    }
    else
    {
        ScalerSetBit_EXINT(P66_37_CC_HW_Z_CFG, ~_BIT0, _BIT0);
    }
}
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcVconnControl_EXINT0(bit bAction)
{
    if(bAction == _ON)
    {
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcFWModeTurnOffVconn_EXINT0(void)
{
    // [WS116] Ellisys FW Solution, Turn off Vconn below 0.8V within 35ms
    if(GET_TYPE_C_0_PD_POWER_ROLE() == _PD_POWER_SRC)
    {
        // 1. [0x665A/5B] Setting FW Mode Rp Resistor (36K / 12K / 4.7K) and Vconn pin to Rd
        if(GET_TYPE_C_0_CC_ORIENTATION() == _TYPE_C_ORIENTATION_UNFLIP)
        {
            if(ScalerGetBit_EXINT(P66_37_CC_HW_Z_CFG, (_BIT7 | _BIT6)) == 0x00)
            {
                // Setting Rp 36K
                ScalerTypeC0CcFWModeSetResistor(_TYPE_C_PIN_CC1, _TYPE_C_FW_MODE_RP_DEF);
            }
            else if(ScalerGetBit_EXINT(P66_37_CC_HW_Z_CFG, (_BIT7 | _BIT6)) == _BIT6)
            {
                // Setting Rp 12K
                ScalerTypeC0CcFWModeSetResistor(_TYPE_C_PIN_CC1, _TYPE_C_FW_MODE_RP_1_5);
            }
            else if(ScalerGetBit_EXINT(P66_37_CC_HW_Z_CFG, (_BIT7 | _BIT6)) == _BIT7)
            {
                // Setting Rp 4.7K
                ScalerTypeC0CcFWModeSetResistor(_TYPE_C_PIN_CC1, _TYPE_C_FW_MODE_RP_3_0);
            }

            // Assert Rd to ensure Vconn Voltage below 0.8V
            ScalerTypeC0CcFWModeSetResistor(_TYPE_C_PIN_CC2, _TYPE_C_FW_MODE_RD);
        }
        else if(GET_TYPE_C_0_CC_ORIENTATION() == _TYPE_C_ORIENTATION_FLIP)
        {
            if(ScalerGetBit_EXINT(P66_37_CC_HW_Z_CFG, (_BIT7 | _BIT6)) == 0x00)
            {
                // Setting Rp 36K
                ScalerTypeC0CcFWModeSetResistor(_TYPE_C_PIN_CC2, _TYPE_C_FW_MODE_RP_DEF);
            }
            else if(ScalerGetBit_EXINT(P66_37_CC_HW_Z_CFG, (_BIT7 | _BIT6)) == _BIT6)
            {
                // Setting Rp 12K
                ScalerTypeC0CcFWModeSetResistor(_TYPE_C_PIN_CC2, _TYPE_C_FW_MODE_RP_1_5);
            }
            else if(ScalerGetBit_EXINT(P66_37_CC_HW_Z_CFG, (_BIT7 | _BIT6)) == _BIT7)
            {
                // Setting Rp 4.7K
                ScalerTypeC0CcFWModeSetResistor(_TYPE_C_PIN_CC2, _TYPE_C_FW_MODE_RP_3_0);
            }

            // Assert Rd to ensure Vconn Voltage below 0.8V
            ScalerTypeC0CcFWModeSetResistor(_TYPE_C_PIN_CC1, _TYPE_C_FW_MODE_RD);
        }
...
// Output Value : None
//--------------------------------------------------
bit ScalerTypeC0CcAttachedSetting_EXINT0(EnumTypeCPDState *penumPdFwState)
{
    bit bPass = _FALSE;

    switch(GET_TYPE_C_0_CC_HW_STATE())
    {
        case _TYPE_C_CC_HW_FSM_ATTACHED_SRC:

            *penumPdFwState = _PD_SRC_START_UP;

            if(ScalerGetBit_EXINT(P66_33_CC_HW_FSM, (_BIT7)) == _BIT7)
            {   // Un-Flip : PD BMC Signal : CC1 / Vconn : CC2
                SET_TYPE_C_0_CC_ORIENTATION(_TYPE_C_ORIENTATION_UNFLIP);

                // Get [0x662A][3:0], Check if Ra is Presented on CC2
                if((ScalerGetBit_EXINT(P66_2A_CC_ANA_DET_R, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) & (_BIT3 | _BIT2)) == 0x00)
                {
                    SET_TYPE_C_0_CC_RA_EXIST();
                }

                // [0x660C] Disconnect CC2 Detection Channel and CC2 Channel and BMC Channel
                // [6] tpc_cc2_det_en     = _TYPE_C_DISCONNECT
                // [4] tpc_cc2_channel_en = _TYPE_C_DISCONNECT
                // [2] tpc_cc2_en         = _TYPE_C_DISCONNECT (Not Needed)
                ScalerSetBit_EXINT(P66_0C_TPC_CC_CTRL, ~(_BIT6 | _BIT4 | _BIT2), 0x00);

                bPass = _TRUE;
            }
            else
            {   // Flip : PD BMC Signal : CC2 / Vconn : CC1
                SET_TYPE_C_0_CC_ORIENTATION(_TYPE_C_ORIENTATION_FLIP);

                // Check if Ra is Presented on CC2
                if((ScalerGetBit_EXINT(P66_2A_CC_ANA_DET_R, (_BIT7 | _BIT6 | _BIT5 | _BIT4)) & (_BIT7 | _BIT6)) == 0x00)
                {
                    SET_TYPE_C_0_CC_RA_EXIST();
                }

                // [0x660C] Disconnect CC1 Detection Channel and CC1 Channel and BMC Channel
                // [7] tpc_cc1_det_en     = _TYPE_C_DISCONNECT
                // [5] tpc_cc1_channel_en = _TYPE_C_DISCONNECT
                // [3] tpc_cc1_en         = _TYPE_C_DISCONNECT (Not Needed)
                ScalerSetBit_EXINT(P66_0C_TPC_CC_CTRL, ~(_BIT7 | _BIT5 | _BIT3), 0x00);

                bPass = _TRUE;
            }

            if(GET_TYPE_C_0_CC_RA_EXIST() == _TRUE)
            ...
            ScalerTypeC0PmicVbusDischarge_EXINT0(_OFF);

            // Cancel WDTimerEvent (_UNATTACH_VBUS_DISCHARGE_TIMER)
            ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_CC_0_CHECK_VBUS_0V);

            // If SRC, Turn On Vbus to vSafe5V (500 * 10mV) / Current = Default Current (10mA)
            ScalerTypeC0PmicSetting_EXINT0(_ON, _PD_POWER_SRC, _PD_FIX_SUPPLY_PDO, 500, GET_TYPE_C_0_OCP_CURRENT(), _PD_PEAK_CUR_NONE);
#endif

#if(_TYPE_C_EIZO_CUSTOMIZED_PMIC_SIC437A_SUPPORT == _ON)
            // [Eizo] SIC437A Customized Power Control : SRC/SNK Attach
            ScalerTypeC0PmicSic437aAttachProc_EXINT0(_TYPE_C_POWER_SRC);
#endif
        ...
        case _TYPE_C_CC_HW_FSM_ATTACHED_SNK:
            ...
            *penumPdFwState = _PD_SNK_START_UP;
            ...
            ScalerTypeCAdcOnOffCompIntSetting_EXINT0(_TYPE_C_CHANNEL_0_VMON, _OFF, ScalerTypeCAdcCorrectionToRawData_EXINT0(_TYPE_C_CHANNEL_0_VMON, (367 / 10)));
...
#if(_TYPE_C_EIZO_CUSTOMIZED_PMIC_SIC437A_SUPPORT == _ON)
            // [Eizo] SIC437A Customized Power Control : SRC/SNK Attach
            ScalerTypeC0PmicSic437aAttachProc_EXINT0(_TYPE_C_POWER_SNK);
#endif
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcSetBMC_EXINT0(bit bAction)
{
    if(bAction == _ENABLE)
    {
        ...
        ScalerSetBit_EXINT(P66_25_BMC_TX_RX_CFG, ~(_BIT7 | _BIT6 | _BIT3), (_BIT7 | _BIT3));
        ...
        if(GET_TYPE_C_0_CC_ORIENTATION() == _TYPE_C_ORIENTATION_UNFLIP)
        {
            ...
            // [2] tpc_cc2_en = _TYPE_C_DISCONNECT (Not Needed)
            ScalerSetBit_EXINT(P66_0C_TPC_CC_CTRL, ~(_BIT3 | _BIT2), _BIT3);
        }
        else
        {
            // [3] tpc_cc1_en = _TYPE_C_DISCONNECT  (Not Needed)
            // [2] tpc_cc2_en = _TYPE_C_CONNECT
            ScalerSetBit_EXINT(P66_0C_TPC_CC_CTRL, ~(_BIT3 | _BIT2), _BIT2);
        }
    }
    else
    {   // [0x6625] Disable BMC Channel
        ...
        ScalerSetBit_EXINT(P66_25_BMC_TX_RX_CFG, ~(_BIT7 | _BIT6), 0x00);

        // [0x660C] CC1/CC2 Disconnect to BMC Channel
        // [3] tpc_cc1_en = _TYPE_C_DISCONNECT
        // [2] tpc_cc2_en = _TYPE_C_DISCONNECT
        ScalerSetBit_EXINT(P66_0C_TPC_CC_CTRL, ~(_BIT3 | _BIT2), 0x00);
    }
}
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcVbusDetCtrl_EXINT0(bit bAction)
{
...
// Output Value : None
//--------------------------------------------------
BYTE ScalerTypeC0CcDetResistor_EXINT0(void)
{
    // Only Update Resistor of CC Pin, Not Vconn Pin (Flip : [7:4] CC1, Un-Flip : [3:0] CC2)
    if(GET_TYPE_C_0_CC_ORIENTATION() == _TYPE_C_ORIENTATION_UNFLIP)
    {
        if(ScalerGetBit_EXINT(P66_2A_CC_ANA_DET_R, (_BIT7 | _BIT6 | _BIT5 | _BIT4)) == 0xF0)
        {
    ...
    {
        if(ScalerGetBit_EXINT(P66_2A_CC_ANA_DET_R, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) == 0x0F)
        {
...
//--------------------------------------------------
// Description  : Get TypeC 0 CC HW State
// Input Value  : None
// Output Value : _TRUE / _FALSE
//--------------------------------------------------
BYTE ScalerTypeC0CcGetHWState_EXINT0(void)
{
    return GET_TYPE_C_0_CC_HW_STATE();
}

//--------------------------------------------------
// Description  : Get TypeC 0 CC is in TryWait.SNK State
// Input Value  : None
// Output Value : _TRUE / _FALSE
//--------------------------------------------------
bit ScalerTypeC0CcGetTryWaitSNKState_EXINT0(void)
{
    return GET_TYPE_C_0_CC_TRY_WAIT_SNK_STATE();
}
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcFWModeTrySrcAttached_EXINT0(void)
{
    EnumTypeCPcbPort enumTypeCPcbPort = ScalerTypeCxPcbxMapping_EXINT0(_EMB_TYPE_C_0);
#if(_EMB_TYPE_C_0_PD_BEHAVIOR_TYPE == _TYPE_C_DP_RX)
    EnumInputPort enumInputPort = ScalerTypeCPcbPortConvertToDx_EXINT0(enumTypeCPcbPort);
    ...
    ScalerSetBit_EXINT(P66_58_DBCC_SNK_DET_COND, ~(_BIT4 | _BIT3 | _BIT1), 0x00);

    // Disable Both CC Detect Equal Condition
    ScalerSetBit_EXINT(P66_40_CC_FW_DET_CTRL, ~_BIT7, 0x00);

    // Popup Digital Detect Result
    ScalerSetBit_EXINT(P66_2C_CC_R_CHG_INT, ~(_BIT7 | _BIT5 | _BIT0), _BIT0);

    // [0x662B] [5:3]: CC1 = Rp
    if((ScalerGetByte_EXINT(P66_2B_CC_DIG_DET_R) & (_BIT5 | _BIT4 | _BIT3)) >= (_BIT4 | _BIT3))
    {
        // Un-Flip : PD BMC Signal : CC1 / Vconn : CC2
        SET_TYPE_C_0_CC_ORIENTATION(_TYPE_C_ORIENTATION_UNFLIP);
    ...
    // [0x6635] Set HW State to Attached.SNK
    ScalerSetBit_EXINT(P66_30_CC_HW_MODE_CTRL, ~(_BIT6 | _BIT3 | _BIT1), 0x00);
    ScalerSetBit_EXINT(P66_35_CC_HW_STATE_TRANSIT, ~(_BIT7 | _BIT6 | _BIT5 | _BIT4), _BIT6);

    // [0x6630] : [7] tpc_cc_mode_sel, [6] hw_mode_fsm_rst, [3] hw_done_irq, [1] hw_unattach_irq, [0] hw_unattach_irq_en
    // Change Back to HW Mode, Clear (hw_done_irq) & (hw_unattach_irq), Enable hw_unattach_irq_en
    ScalerSetBit_EXINT(P66_30_CC_HW_MODE_CTRL, ~(_BIT7 | _BIT6 | _BIT3 | _BIT1 | _BIT0), (_BIT7 | _BIT6 | _BIT3 | _BIT1 | _BIT0));
}
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcUnattachFlow_EXINT0(void)
{
    ...
    ScalerTypeCAdcUnattachReset_EXINT0(_EMB_TYPE_C_0);
#if(_EMB_TYPE_C_0_PMIC_TYPE != _PMIC_NONE)
    ScalerTypeC0PmicReset_EXINT0();
#endif
    ScalerTypeC0PdReset_EXINT0();
    // FW Solution : Samsung S8 Clear Flag When CC Unattach (Not Cleared When PD Reset)
    ScalerTypeC0PdClrInteropS8Flag_EXINT0();
    ScalerTypeC0CcUnattachReset_EXINT0();

    // 2. Setting For Connection
    ScalerTypeC0CcConnectionSet_EXINT0();

    // 3. State Transition
    SET_TYPE_C_0_CC_FW_STATE(_TYPE_C_CC_EMB_FW_FSM_CONNECTING);

    // 4. [3] Clear IRQ hw_done_irq  = 1, Start HW CC Connection Detection
    ScalerTypeC0CcConnectionStart_EXINT0();
}
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcUnattachReset_EXINT0(void)
{
// =============================== Control By Power Delivery ??? ===============================
    ...
    {
        ScalerTypeC0CcFastRoleSwapControl_EXINT0(_DISABLE);
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcConnectionSet_EXINT0(void)
{
    ...
    ScalerSetBit_EXINT(P66_37_CC_HW_Z_CFG, ~(_BIT7 | _BIT6), (_TYPE_C_SET_RP_DEF << 6));
    SET_TYPE_C_0_CC_DEF_CUR(90);  // 90 * 10mA = 900mA
#elif(_EMB_TYPE_C_0_SRC_DEF_CUR == _TYPE_C_SRC_DEF_1P5A)
    ScalerSetBit_EXINT(P66_37_CC_HW_Z_CFG, ~(_BIT7 | _BIT6), (_TYPE_C_SET_RP_1P5A << 6));
    SET_TYPE_C_0_CC_DEF_CUR(150); // 150 * 10mA = 1.5A
#else
    ScalerSetBit_EXINT(P66_37_CC_HW_Z_CFG, ~(_BIT7 | _BIT6), (_TYPE_C_SET_RP_3P0A << 6));
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcConnectionStart_EXINT0(void)
{
#if(_TYPE_C_0_CC_CONNECT_MODE == _TYPE_C_HW_MODE)
    // [0x6630] : [3] hw_done_irq, [2] hw_done_irq_en, [1] hw_unattach_irq, [0] hw_unattach_irq_en
    // Enable hw_done_irq, Disable hw_unattach_irq and Clear IRQs
    ScalerSetBit_EXINT(P66_30_CC_HW_MODE_CTRL, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT2 | _BIT1));
    ...
    if((GET_TYPE_C_0_CC_PORT_ROLE() == _TYPE_C_PORT_DRP) && (GET_TYPE_C_0_CC_DRP_PREFER() == _TYPE_C_DRP_TRY_SRC))
    {
        ...
        ScalerSetBit_EXINT(P66_58_DBCC_SNK_DET_COND, ~(_BIT3 | _BIT1 | _BIT0), (_BIT1 | _BIT0));
    }
#else
    // FW_MODE Reset
    ScalerSetBit_EXINT(P66_30_CC_HW_MODE_CTRL, ~(_BIT7 | _BIT6 | _BIT5 | _BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT1));
#endif
}
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcFastRoleSwapControl_EXINT0(bit bAction)
{
    if(bAction == _ENABLE)
    {
        if(ScalerTypeC0PdGetPowerRole_EXINT0() == _TYPE_C_POWER_SRC)
        {
            // [0x6666] Clear and Enable SRC FR_SWAP INT
            // [7] src_frs_done_int    = 1
            // [6] src_frs_done_int_en = _ENABLE
            ScalerSetBit_EXINT(P66_66_SRC_FRS_INT, ~_BIT7, _BIT7);
            ScalerSetBit_EXINT(P66_66_SRC_FRS_INT, ~(_BIT7 | _BIT6), _BIT6);

            // [0x6660] Turn On Source-Side FR_SWAP
            // [7] src_frs_en = _ENABLE
            ScalerSetBit_EXINT(P66_60_FRS_CTRL, ~(_BIT7 | _BIT5), _BIT7);
        ...
        else
        {
            ...
            ScalerSetBit_EXINT(P66_67_SNK_FRS_INT, ~(_BIT7 | _BIT6 | _BIT3 | _BIT2), (_BIT7 | _BIT6 | _BIT3 | _BIT2));

            // [0x6660] Turn On Sink-Side FR_SWAP
            // [6] snk_frs_en = _ENABLE
            ScalerSetBit_EXINT(P66_60_FRS_CTRL, ~(_BIT6 | _BIT5), _BIT6);
    ...
    {   // [0x6666] Disable SRC FR_SWAP INT
        ...
        ScalerSetBit_EXINT(P66_66_SRC_FRS_INT, ~(_BIT7 | _BIT6), 0x00);

        // [0x6667] Disable SNK FR_SWAP INT
        // [6] snk_frs_done_int_en = _DISABLE
        // [2] snk_frs_tri_int_en  = _DISABLE
        ScalerSetBit_EXINT(P66_67_SNK_FRS_INT, ~(_BIT7 | _BIT6 | _BIT3 | _BIT2), 0x00);

        // [0x6660] Turn Off Source/Sink-Side FR_SWAP
        // [7] src_frs_en = _DISABLE
        // [6] snk_frs_en = _DISABLE
        ScalerSetBit_EXINT(P66_60_FRS_CTRL, ~(_BIT7 | _BIT6 | _BIT5), 0x00);

        // [0x6668] Turn Off Fast-Role Swap
        ScalerSetBit_EXINT(P66_68_FRS_BYPASS_MODE, ~_BIT7, 0x00);
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC0CcToolWarningUIControl_EXINT0(bit bAction)
{
