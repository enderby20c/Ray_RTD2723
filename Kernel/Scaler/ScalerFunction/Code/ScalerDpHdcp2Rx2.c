...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerDpHdcp2Rx2.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
//----------------------------------------------------------------------------
volatile BYTE g_ucDpHdcp2Rx2Ctrl0;
volatile BYTE g_ucDpHdcp2Rx2Ctrl1;
BYTE g_pucDpHdcp2Rx2dKey0[16];
BYTE g_pucDpHdcp2Rx2dKey1[16];
BYTE g_pucDpHdcp2Rx2Km[16];
BYTE g_pucDpHdcp2Rx2Rtx[8];
BYTE g_pucDpHdcp2Rx2TxCaps[3];
BYTE g_pucDpHdcp2Rx2Rn[8];
BYTE g_pucDpHdcp2Rx2Rrx[8];
BYTE g_pucDpHdcp2Rx2VPrime[20];
BYTE g_pucDpHdcp2Rx2Caps[3];

#if(_DP_TX_SUPPORT == _ON)
volatile StructDpHdcp2RxAuthInfo g_stDpHdcp2Rx2AuthInfo;
#endif
...
//****************************************************************************
//--------------------------------------------------
// Description  : HDCP 2.2 Handler Process for DP Rx2
// Input Value  : None
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2Handler(void)
{
    BYTE pucTemp1[16] = {0};
    BYTE pucTemp2[16] = {0};
    BYTE pucTemp3[16] = {0};
    ...
    if(GET_DP_HDCP2_RX2_AKE_INIT_RECEIVED() == _TRUE)
    {
        ...
        {
            SET_DP_HDCP2_RX2_REPEATER_AUTH_1_START();
        }

        // Clear DPCD Repeater Info
        ScalerDpHdcp2Rx2ClearDpcdRepeaterInfo();
#endif
    ...
    if(GET_DP_HDCP2_RX2_AKE_NO_STORED_KM_RECEIVED() == _TRUE)
    {
        if(ScalerDpHdcp2Rx2CaptureKpubKm() == _TRUE)
        {
            // Decrypt Km by FW to Get 128 bit Km
            if(ScalerHdcp2RxRsaOaepCalculate(pucTemp1) == _TRUE)
            {
                ScalerHdcp2DkeyCalculate(_HDCP_2_2_DKEY_0, g_pucDpHdcp2Rx2Rtx, g_pucDpHdcp2Rx2Rrx, pucTemp1, g_pucDpHdcp2Rx2Rn);

                memcpy(pucTemp2, (volatile BYTE xdata *)P63_81_HDCP_AES_OUT_15, 16);

                ScalerHdcp2DkeyCalculate(_HDCP_2_2_DKEY_1, g_pucDpHdcp2Rx2Rtx, g_pucDpHdcp2Rx2Rrx, pucTemp1, g_pucDpHdcp2Rx2Rn);

                memcpy(pucTemp3, (volatile BYTE xdata *)P63_81_HDCP_AES_OUT_15, 16);

                ScalerHdcp2Proc2(g_pucDpHdcp2Rx2Km, pucTemp1);
                ScalerHdcp2Proc2(g_pucDpHdcp2Rx2dKey0, pucTemp2);
                ScalerHdcp2Proc2(g_pucDpHdcp2Rx2dKey1, pucTemp3);

                if(ScalerHdcp2HPrimeCalculate(pucTemp2, pucTemp3, g_pucDpHdcp2Rx2Rtx, g_pucDpHdcp2Rx2TxCaps, g_pucDpHdcp2Rx2Caps) == _TRUE)
                {
                    ScalerDpHdcp2Rx2CalculateKmKh();
                }
            }
        }
    }

    if(GET_DP_HDCP2_RX2_AKE_STORED_KM_RECEIVED() == _TRUE)
    {
    ...
    if(GET_DP_HDCP2_RX2_LC_INIT_RECEIVED() == _TRUE)
    {
    ...
    if(GET_DP_HDCP2_RX2_SKE_SEND_EKS_RECEIVED() == _TRUE)
    {
        ...
        if(GET_DP_HDCP2_RX2_REPEATER_SUPPORT() == _TRUE)
        {
            SET_DP_HDCP2_RX2_REPEATER_AUTH_2_START();
        }
#endif
    ...
    if(GET_DP_HDCP2_RX2_STREAM_MANAGE_RECEIVED() == _TRUE)
    {
        CLR_DP_HDCP2_RX2_STREAM_MANAGE_RECEIVED();

#if(_DP_TX_SUPPORT == _ON)
        SET_DP_HDCP2_RX2_CONTENT_STREAM_MANAGEMENT();
#endif
    }
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2ResetProc(void)
{
    if(GET_DP_HDCP2_RX2_AUTHENTICATION_DONE() == _TRUE)
    {
        EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);
        ...
        ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX2_AKE_INIT);

        ScalerDpHdcp2Rx2ClearDpcdReadValue(_HDCP22_RX_H_PRIME, 32);
        ScalerDpHdcp2Rx2ClearDpcdReadValue(_HDCP22_RX_L_PRIME, 32);
        ScalerDpHdcp2Rx2ClearDpcdReadValue(_HDCP22_RX_M_PRIME, 32);
        ScalerDpHdcp2Rx2ClearDpcdReadValue(_HDCP22_RX_PAIRING_INFO, 16);
        ...
        ScalerDpHdcp2Rx2ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_IDLE);

        ScalerDpHdcp2Rx2ClearDpcdReadValue(_HDCP22_RX_V_PRIME, 190);
#endif
    }
}
...
//--------------------------------------------------
bit ScalerDpHdcp2Rx2CaptureKpubKm(void)
{
    BYTE ucIndex = 0x00;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2CalculateKmKh(void)
{
    BYTE ucIndex = 0x00;
    BYTE pucSHA256[32] = {0};
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);
    ...
    SET_DP_HDCP2_RX2_WAITING_H_PRIME_CPIRQ_ASSERTED();

    // Initialize Pairing Available CPIRQ Flag To Record Pairing Available CPIRQ Asserted Status
    CLR_DP_HDCP2_RX2_RECORD_PAIRING_INFO_CPIRQ_NO_ASSERTED();

    SET_DP_HDCP2_RX2_AKE_NO_STORED_KM_DONE();
    ...
    CLR_DP_HDCP2_RX2_WAITING_H_PRIME_CPIRQ_ASSERTED();

    // Assert Pairing Available CPIRQ If No Asserting in Pairing Info INT State
    if(GET_DP_HDCP2_RX2_RECORD_PAIRING_INFO_CPIRQ_NO_ASSERTED() == _TRUE)
    {
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2CancelTimerEvent(void)
{
    ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX2_H_PRIME);
    ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX2_SKE);

#if(_DP_TX_SUPPORT == _ON)
    CLR_DP_HDCP2_RX2_REAUTH_LONG_HPD();

    ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX2_M_PRIME);
    ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX2_AKE_INIT);
#endif
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2ClearDpcdReadValue(BYTE ucRSAType, WORD usLength)
{
    ...
    ScalerDpAuxRx2DpcdGroupReset(0x06, HIBYTE(usDpcdAddress), LOBYTE(usDpcdAddress), usLength);
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2RepeaterHandler(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);

    if(ScalerDpHdcpRxGetHdcpMode(enumInputPort) == _HDCP_22)
    {
        if(GET_DP_HDCP2_RX2_CAPABLE_SUPPORT() == _TRUE)
        {
            if(GET_DP_HDCP2_RX2_REPEATER_SUPPORT() == _TRUE)
            {
                if(GET_DP_HDCP2_RX2_REPEATER_AUTH_1_START() == _TRUE)
                {
                    DebugMessageHDCP2("DPRX2 HDCP2 - Get AKE INIT", 0);

                    ScalerTimerCancelTimerEvent(_SCALER_TIMER_EVENT_DP_RX2_HDCP2_CPIRQ_EVENT);

                    CLR_DP_HDCP2_RX2_AUTH_CPIRQ_TIMRE();

#if(_DP_HDCP14_RX2_REPEATER_SUPPORT == _ON)
                    ScalerDpHdcp14Rx2ChangeAuthState(_DP_RX_HDCP_STATE_IDLE);
#endif
                ...
                if(GET_DP_RX_HDCP2_UPSTREAM_PORT() == _HDCP_D2_PORT)
                {
                    ...
                    {
                        if((ScalerDpMacDphyRxHdcpCheckValid(enumInputPort) == _TRUE) && (GET_DP_HDCP2_RX2_AUTH_CPIRQ_TIMRE() == _FALSE))
                        {
                            if(ScalerDpMacDphyRxHdcpCheckLvp(enumInputPort) == _FALSE)
                            {
                                SET_DP_HDCP2_RX2_AUTH_CPIRQ_TIMRE();

                                ScalerDpMacDphyRxCpIRQ(enumInputPort, _DP_HDCP_BSTATUS_LINK_INTEGRITY_FAIL);

                                ScalerTimerActiveTimerEvent(SEC(0.02), _SCALER_TIMER_EVENT_DP_RX2_HDCP2_CPIRQ_EVENT);
                            }
                        }
                    }

                    if((ScalerDpHdcp2Rx2ReauthEventCheck() == _TRUE) && (GET_DP_HDCP2_RX2_AUTH_STATE() != _DP_RX_HDCP2_REPEATER_STATE_IDLE))
                    {
                        if(GET_DP_HDCP2_RX2_AUTH_STATE() != _DP_RX_HDCP2_REPEATER_STATE_AUTH_DONE)
                        {
                            ScalerDpHdcp2Rx2ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_AUTH_1);

                            SET_DP_HDCP2_RX2_REPEATER_DFP_REAUTH();
                        }
                        else
                        {
                            CLR_DP_HDCP2_RX2_REPEATER_DFP_REAUTH();

                            ScalerDpHdcp2Rx2ReAuthProcess(enumInputPort);
                        }
                    }
                }

                if((GET_DP_HDCP2_RX2_AUTH_STATE() != _DP_RX_HDCP2_REPEATER_STATE_IDLE) && (ScalerDpAuxRxGetLTStatus(enumInputPort) == _DP_LINK_TRAINING_NONE))
                {
                    ScalerDpHdcp2Rx2ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_IDLE);
                }
            }
    ...
    switch(GET_DP_HDCP2_RX2_AUTH_STATE())
    {
        case _DP_RX_HDCP2_REPEATER_STATE_IDLE:
            ...
            if(GET_DP_HDCP2_RX2_AUTH_STATE_CHANGE() == _TRUE)
            {
                ...
                memset(g_pucDpHdcpRx2ECFBackup, 0, sizeof(g_pucDpHdcpRx2ECFBackup));

                CLR_DP_HDCP2_RX2_AUTH_STATE_CHANGE();
                CLR_DP_HDCP2_RX2_AUTH_REPEATER_PROC_TIMEOUT();
                CLR_DP_HDCP2_RX2_AUTH_CPIRQ_TIMRE();
                CLR_DP_HDCP2_RX2_CONTENT_STREAM_MANAGEMENT();
                CLR_DP_HDCP2_RX2_REPEATER_DFP_REAUTH();

                ScalerTimerCancelTimerEvent(_SCALER_TIMER_EVENT_DP_RX2_HDCP2_REPEATER_PROC_TIMEOUT);

                SET_DP_HDCP2_RX2_UPSTREAM_EVENT(_DP_RX_HDCP2_UPSTREAM_AUTH_EVENT_STOP_ENCRYPTION);
            }

            if((ScalerDpHdcpRxGetHdcpMode(enumInputPort) == _HDCP_22) && (GET_DP_HDCP2_RX2_REPEATER_SUPPORT() == _TRUE))
            {
                if(GET_DP_HDCP2_RX2_REPEATER_AUTH_1_START() == _TRUE)
                {
                    CLR_DP_HDCP2_RX2_REPEATER_AUTH_1_START();
        ...
        case _DP_RX_HDCP2_REPEATER_STATE_AUTH_1:
            ...
            if(GET_DP_HDCP2_RX2_AUTH_STATE_CHANGE() == _TRUE)
            {
                DebugMessageHDCP2("DPRX2 HDCP2 - Auth 1", 0);
                ...
                if(GET_DP_RX_HDCP2_UPSTREAM_PORT() == _HDCP_D2_PORT)
                {
                    ...
                    {
                        DebugMessageHDCP2("DPRX2 HDCP2 - To DPTX HDCP2", 0);

                        // Issue event to make DPTX start HDCP2 handshake.
                        SET_DP_HDCP2_RX2_UPSTREAM_EVENT(_DP_RX_HDCP2_UPSTREAM_AUTH_EVENT_START_HANDSHAKE);

                        ScalerTimerActiveTimerEvent(SEC(6), _SCALER_TIMER_EVENT_DP_RX2_HDCP2_REPEATER_PROC_TIMEOUT);
                    }
                    else if(GET_DP_TX_HDCP_DOWNSTREAM_SUPPORTED() == _TRUE)
                    {
                        DebugMessageHDCP2("DPRX2 HDCP2 - To DPTX HDCP1", 0);

                        // Issue event to make DPTX start HDCP handshake.
                        SET_DP_HDCP14_RX2_UPSTREAM_EVENT(_DP_RX_HDCP_UPSTREAM_AUTH_EVENT_R0_PASS);
                    ...
                    else
                    {
                        ...
                        ScalerDpHdcp2Rx2ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_AUTH_2);
                    }
                }
                else
                {
                    ...
                    ScalerDpHdcp2Rx2ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_AUTH_FAKE);
                }
            }
            ...
            {
                DebugMessageHDCP2("DPRX2 HDCP2 - TX PASS", 0);
            ...
            else if(GET_DP_HDCP2_RX2_AUTH_REPEATER_PROC_TIMEOUT() == _TRUE)
            {
                CLR_DP_HDCP2_RX2_AUTH_REPEATER_PROC_TIMEOUT();

                if((GET_DP_HDCP2_TX_CAPABLE_SUPPORTED() == _TRUE) && (GET_DP_HDCP2_TX_VERSION_SUPPORTED() == _TRUE))
                {
                    SET_DP_HDCP2_RX2_UPSTREAM_EVENT(_DP_RX_HDCP2_UPSTREAM_AUTH_EVENT_TIMEOUT);
                }
                else if(GET_DP_TX_HDCP_DOWNSTREAM_SUPPORTED() == _TRUE)
                {
                    SET_DP_HDCP14_RX2_UPSTREAM_EVENT(_DP_RX_HDCP_UPSTREAM_AUTH_EVENT_TIMEOUT);
                }

                if(GET_DP_HDCP2_RX2_REPEATER_DFP_REAUTH() == _TRUE)
                {
                    CLR_DP_HDCP2_RX2_REPEATER_DFP_REAUTH();
                ...
                ScalerDpHdcp2Rx2ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_AUTH_DONE);
            }
        ...
        case _DP_RX_HDCP2_REPEATER_STATE_AUTH_2:
            ...
            if(GET_DP_HDCP2_RX2_AUTH_STATE_CHANGE() == _TRUE)
            {
                DebugMessageHDCP2("DPRX2 HDCP2 - Auth 2", 0);

                CLR_DP_HDCP2_RX2_AUTH_STATE_CHANGE();
            }

            if(GET_DP_HDCP2_RX2_REPEATER_AUTH_2_START() == _TRUE)
            {
                CLR_DP_HDCP2_RX2_REPEATER_AUTH_2_START();

                if(ScalerDpHdcp2Rx2CheckRxInfo() == _TRUE)
                {
                    ScalerDpHdcp2Rx2LoadReceiverIdList();

                    ScalerDpHdcp2Rx2VComputeProc();
                }

                DebugMessageHDCP2("DPRX2 HDCP2 - V Ready", 0);
        ...
        case _DP_RX_HDCP2_REPEATER_STATE_AUTH_FAKE:
            ...
            if(GET_DP_HDCP2_RX2_AUTH_STATE_CHANGE() == _TRUE)
            {
                DebugMessageHDCP2("DPRX2 HDCP2 - Fake", 0);
            ...
            if(GET_DP_HDCP2_RX2_REPEATER_AUTH_2_START() == _TRUE)
            {
                ...
                // Fake Authentication
                ScalerDpHdcp2Rx2FakeAuthenProc();
        ...
        case _DP_RX_HDCP2_REPEATER_STATE_AUTH_DONE:
            ...
            if(GET_DP_HDCP2_RX2_AUTH_STATE_CHANGE() == _TRUE)
            {
                DebugMessageHDCP2("DPRX2 HDCP2 - Done", 0);

                CLR_DP_HDCP2_RX2_AUTH_STATE_CHANGE();
            }

#if(_DP_MST_SUPPORT == _ON)
            ScalerDpHdcp2Rx2ChangeECFCheck();
#endif
            break;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2ChangeECFCheck(void)
{
    ...
    if((GET_DP_RX_TARGET_MST_PORT() == _DP_MST_RX2_PORT) && (GET_DP_RX_HDCP_UPSTREAM_PORT() == _HDCP_D2_PORT))
    {
        for(ucDaisyStreamCnt = 0; ucDaisyStreamCnt < _HW_DP_MST_DAISY_ST_NUM_MAX; ucDaisyStreamCnt++)
        {
            ...
            if(ucHdcpTimeSlotEncValue != g_pucDpHdcpRx2ECFBackup[ucDaisyStreamCnt])
            {
                g_pucDpHdcpRx2ECFBackup[ucDaisyStreamCnt] = ucHdcpTimeSlotEncValue;
    ...
    if(bECFChanged == _TRUE)
    {
        if((GET_DP_HDCP2_TX_CAPABLE_SUPPORTED() == _TRUE) && (GET_DP_HDCP2_TX_VERSION_SUPPORTED() == _TRUE) && (GET_DP_TX_STREAM_SOURCE() == _DP_TX_SOURCE_MST))
        {
            ...
            SET_DP_HDCP2_RX2_UPSTREAM_EVENT(_DP_RX_HDCP2_UPSTREAM_AUTH_EVENT_ECF_CHANGE);
        }
        else
        {
            ...
            {
                // Issue event to inform DPTX HDCP ECF Changed.
                SET_DP_HDCP14_RX2_UPSTREAM_EVENT(_DP_RX_HDCP_UPSTREAM_AUTH_EVENT_ECF_CHANGE);
            }
        }
    }
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2ChangeAuthState(EnumDpHdcp2RxRepeaterAuthState enumHdcp2AuthState)
{
    SET_DP_HDCP2_RX2_AUTH_STATE(enumHdcp2AuthState);
    SET_DP_HDCP2_RX2_AUTH_STATE_CHANGE();
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2SetRepeaterProcTimeout(void)
{
    SET_DP_HDCP2_RX2_AUTH_REPEATER_PROC_TIMEOUT();
}
...
// Output Value : None
//--------------------------------------------------
bit ScalerDpHdcp2Rx2CheckRxInfo(void)
{
    BYTE pucRxInfo[2] = {0};
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2LoadReceiverIdList(void)
{
    BYTE ucIndex = 0x00;
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);
    EnumDpAuxRxModeStatus enumDpAuxRxModeStatus = GET_DP_AUX_RX2_MODE();
...
// Output Value : None
//--------------------------------------------------
bit ScalerDpHdcp2Rx2VComputeProc(void)
{
    BYTE ucIndex = 0x00;
    BYTE pucRxInfo[2] = {0};
    BYTE pucSeqNumV[3] = {0};
    ...
    {
        g_pucDpHdcp2Rx2VPrime[ucIndex] = ScalerGetByte(P63_35_HDCP_SHA_0 + ucIndex);

        // DPCD [0x69335 ~ 0x69344]
        ScalerDpAuxRxSetDpcdValue(enumInputPort, 0x06, 0x93, (0x35 + ucIndex), g_pucDpHdcp2Rx2VPrime[ucIndex]);
    }
...
// Output Value : None
//--------------------------------------------------
bit ScalerDpHdcp2Rx2FakeAuthenProc(void)
{
    BYTE ucIndex = 0x00;
    BYTE pucRxInfo[2] = {0};
    BYTE pucSeqNumV[3] = {0};
    ...
    ScalerDpAuxRx2DpcdGroupReset(0x06, 0x93, 0x4A, 150);
...
//--------------------------------------------------
// Description  : Change DP HDCP2.2 Repeater Capable for Rx2 Base On DPTX Clone Status
// Input Value  : InputPort, PortType, Clone Mode or not
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2RepeaterSupportProc(void)
{
    bit bDPRx2Hdcp2SetRepeater = _OFF;
    EnumInputPort enumHdcpInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);

    if(GET_DP_HDCP2_RX2_CAPABLE_SUPPORT() == _FALSE)
    {
        return;
    }
    ...
    if(GET_DP_MAC_TX_DOWNSTREAM_DPPLUG_STATUS() != _DP_UNPLUG)
    {
        if(GET_DP_RX_HDCP2_UPSTREAM_PORT() == _HDCP_D2_PORT)
        {
            bDPRx2Hdcp2SetRepeater = _ON;
        }
    }

    if(bDPRx2Hdcp2SetRepeater != GET_DP_HDCP2_RX2_REPEATER_SUPPORT())
    {
        if(bDPRx2Hdcp2SetRepeater == _ON)
        {
            SET_DP_HDCP2_RX2_REPEATER_SUPPORT();
        }
        else
        {
            CLR_DP_HDCP2_RX2_REPEATER_SUPPORT();
        }

        switch(GET_DP_RX2_HOTPLUG_ASSERT_TYPE())
        {
            case _DP_HPD_NONE:
...
// Output Value : None
//--------------------------------------------------
bit ScalerDpHdcp2Rx2ReauthEventCheck(void)
{
    BYTE ucReauth = 0;
    ...
    if((GET_DP_HDCP2_TX_CAPABLE_SUPPORTED() == _TRUE) && (GET_DP_HDCP2_TX_VERSION_SUPPORTED() == _TRUE))
    {
        ...
        {
            DebugMessageHDCP2("DPRX2 HDCP2 - Get DPTX HDCP2 ReAuth Event", 0);
    ...
    else
    {
        if(GET_DP_TX_HDCP_DOWNSTREAM_SUPPORTED() == _TRUE)
        {
            ...
            {
                DebugMessageHDCP2("DPRX2 HDCP2 - Get DPTX HDCP14 ReAuth Event", 0);
...
//--------------------------------------------------
bit ScalerDpHdcp2Rx2ReAuthStatusCheck(void)
{
    if(GET_DP_HDCP2_RX2_REAUTH_LONG_HPD() == _TRUE)
    {
        return _TRUE;
    }
    else
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2ReAuthProcess(EnumInputPort enumInputPort)
{
    ...
    if((ScalerDpAuxRxGetDpcdBitInfo(enumInputPort, 0x00, 0x01, 0x11, (_BIT1 | _BIT0)) == 0x00) &&
       (GET_DP_HDCP2_RX2_AUTH_STATE() != _DP_RX_HDCP2_REPEATER_STATE_IDLE) &&
       (ScalerDpMacDphyRxHdcpCheckValid(enumInputPort) == _TRUE))
    {
        SET_DP_HDCP2_RX2_REAUTH_LONG_HPD();

        ScalerTimerReactiveTimerEvent(SEC(4), _SCALER_TIMER_EVENT_DP_RX2_HDCP2_LONG_HOTPLUG_EVENT);
    }
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx2ClearDpcdRepeaterInfo(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);
    ...
    if((ScalerDpAuxRxGetDpcdInfo(enumInputPort, 0x06, 0x80, 0x2A) != 0x00) || (ScalerDpAuxRxGetDpcdInfo(enumInputPort, 0x06, 0x80, 0x2B) != 0x00))
    {
        ...
        ScalerDpAuxRx2DpcdGroupReset(0x06, 0x80, 0x14, 20);

        // DPCD [0x68029 ~ 0x6803A]
        ScalerDpAuxRx2DpcdGroupReset(0x06, 0x80, 0x29, 18);
    }
}
