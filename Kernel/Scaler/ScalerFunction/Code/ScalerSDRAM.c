...
{
    ScalerSetByte(P33_A6_M_DOMAIN_DUMMY02, 0x00); // SDRAM
    //==============================
    // DLL Setting
    //==============================
    ...
    ScalerSetDWord(P8F_7C_DPI_CTRL_0_7C, 0x00070476);

    // Enable CS Output One cycle Delay and Initial DQS/DQ Mux table
    ScalerSetDWord(P8E_28_WRLVL_CTRL_28, 0x00000FF0);
    ScalerSetDWord(P8E_28_WRLVL_CTRL_28, ((0x00000FF0 & 0xFFFCCCCF) | (~0xFFFCCCCF & ScalerSdramCalcWrlvlConfig(&(g_pstSdramPLLPHASE[0])))));
    ...
    ScalerSetDWord(P8F_70_PAD_BUS_0_70, 0x00000052);
    ...
    ScalerSetDWord(P8E_B8_DQS_IN_DLY_0_B8, 0x10101010); // slice_0: DQ_03~DQ_00
    ScalerSetDWord(P8E_BC_DQS_IN_DLY_0_BC, 0x10101010); // slice_1: DQ_11~DQ_08
    ScalerSetDWord(P8E_C8_DQS_IN_DLY_1_C8, 0x10101010); // slice_0: DQ_07~DQ_04
    ScalerSetDWord(P8E_CC_DQS_IN_DLY_1_CC, 0x10101010); // slice_1: DQ_15~DQ_12
    // RDQ delay tap - falling
    ScalerSetDWord(P8E_D8_DQS_IN_DLY_2_D8, 0x10101010); // slice_0: DQ_03~DQ_00
    ScalerSetDWord(P8E_DC_DQS_IN_DLY_2_DC, 0x10101010); // slice_1: DQ_11~DQ_08
    ScalerSetDWord(P8E_E8_DQS_IN_DLY_3_E8, 0x10101010); // slice_0: DQ_07~DQ_04
    ScalerSetDWord(P8E_EC_DQS_IN_DLY_3_EC, 0x10101010); // slice_1: DQ_15~DQ_12
    // Apply read delay setting
    ...
    //------------------------------
    // Set IO Pad Driving
    ScalerSetDWord(P8F_14_ODT_TTCP0_SET0_14, 0x03030303); // P-ODT
    ScalerSetDWord(P8F_18_ODT_TTCP1_SET0_18, 0x03030303);
    ScalerSetDWord(P8F_1C_ODT_TTCN0_SET0_1C, 0x03030303); // N-ODT
    ...
    ScalerSetDWord(P90_70_DQS_P_ODT_SEL_70, 0x00000000); // P-ODT/N-ODT coarse tune selection for DQS[0],DQS[1],DQS[2],DQS[3]
    ScalerSetDWord(P90_74_DQS_P_ODT_SEL_74, 0x00000000); // P-ODT/N-ODT coarse tune selection for DQS[0],DQS[1],DQS[2],DQS[3]
    ScalerSetDWord(P90_78_DQS_N_ODT_SEL_78, 0x00000000); // P-ODT/N-ODT coarse tune selection for DQS#[0],DQS#[1],DQS#[2],DQS#[3]
    ScalerSetDWord(P90_7C_DQS_N_ODT_SEL_7C, 0x00000000); // P-ODT/N-ODT coarse tune selection for DQS#[0],DQS#[1],DQS#[2],DQS#[3]
    ...
    // Set fw_set_mode = 'b01
    ScalerSetDWord(P8F_7C_DPI_CTRL_0_7C, 0x00070475);
...
// Input Value  : pstSDRAMPLLPhase
// Output Value : DWORD (32bit) of register setting
//--------------------------------------------------
DWORD ScalerSdramCalcWrlvlConfig(StructSDRAMPLLPI *pstSDRAMPLLPhase)
{
    StructSDRAMPLLPI stTheoPI;
    DWORD ulWrlvlConfig = 0;

    stTheoPI.ucDQS0 = pstSDRAMPLLPhase->ucDQS0 % _TOTAL_THEO_DPI_PI_NUM;
    stTheoPI.ucDQS1 = pstSDRAMPLLPhase->ucDQS1 % _TOTAL_THEO_DPI_PI_NUM;
    stTheoPI.ucDQ0 = pstSDRAMPLLPhase->ucDQ0 % _TOTAL_THEO_DPI_PI_NUM;
    stTheoPI.ucDQ1 = pstSDRAMPLLPhase->ucDQ1 % _TOTAL_THEO_DPI_PI_NUM;
...
void ScalerSdramPLLOutputCtrl(bit bEnable)
{
    ...
    if(bEnable == _ENABLE)
    {
        ...
        // Enable SDR PLL CLK output, and wait until PLL is stable
        ScalerSetDWord(P88_08_PLL_CTL1_08, (0x003F0000 | ((~0xFFFFFFC0 & GET_OESYNC_SETTING(g_pstSdramPLLPHASE[0])))));
    ...
    else // Disable
    {
        ...
        // Disable SDR PLL A CLK output
        ScalerSetDWord(P88_08_PLL_CTL1_08, (0x00000000 | ((~0xFFFFFFC0 & GET_OESYNC_SETTING(g_pstSdramPLLPHASE[0])))));
...
//--------------------------------------------------
// Description  : Set SDRAMPLL Write Phase
// Input Value  : pstDDR2PLLPhase, bDDRSel
// Output Value : None
//--------------------------------------------------
void ScalerSdramSetPLLWritePhase(StructSDRAMPLLPI *pstSDRAMPLLPhase)
{
    pstSDRAMPLLPhase = pstSDRAMPLLPhase;
}
