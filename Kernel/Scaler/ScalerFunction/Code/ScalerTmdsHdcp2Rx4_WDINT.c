...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerTmdsHdcp2Rx4_WDINT.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
//--------------------------------------------------
void ScalerTmdsHdcp2Rx4RxRsaCalculate_WDINT(void)
{
...
// Output Value : None
//--------------------------------------------------
void ScalerTmdsHdcp2Rx4SetRdMsgIDCode_WDINT(BYTE ucValue)
{
...
// Output Value : None
//--------------------------------------------------
void ScalerTmdsHdcp2Rx4SetRdMsgValue_WDINT(WORD usAddress, BYTE *pucReadArray, WORD usLength)
{
    WORD usTemp = 0;
...
// Output Value : None
//--------------------------------------------------
void ScalerTmdsHdcp2Rx4HPrime_WDINT(BYTE *pucTemp1)
{
    BYTE pucTemp2[16] = {0};
    BYTE pucTemp3[16] = {0};
    BYTE pucBackUp[2] = {0};
    ...
    ScalerHdcp2DkeyCalculate_WDINT(_HDCP_2_2_DKEY_0, g_pucTmdsHdcp2Rx4Rtx, g_pucTmdsHdcp2Rx4Rrx, pucTemp1, g_pucTmdsHdcp2Rx4Rn);

    memcpy(pucTemp2, (volatile BYTE xdata *)P63_81_HDCP_AES_OUT_15, 16);

    ScalerHdcp2DkeyCalculate_WDINT(_HDCP_2_2_DKEY_1, g_pucTmdsHdcp2Rx4Rtx, g_pucTmdsHdcp2Rx4Rrx, pucTemp1, g_pucTmdsHdcp2Rx4Rn);

    memcpy(pucTemp3, (volatile BYTE xdata *)P63_81_HDCP_AES_OUT_15, 16);

    if(ScalerHdcp2HPrimeCalculate_WDINT(pucTemp2, pucTemp3, g_pucTmdsHdcp2Rx4Rtx, g_pucTmdsHdcp2Rx4TxCaps, g_pucTmdsMacRx4Caps) == _TRUE)
    {
        __real_memset(g_pucHdcp2Sha256, 0, sizeof(g_pucHdcp2Sha256));

        // Load H prime to Read SRAM, AKE Send H PRIME Message ID = 0x07
        ScalerTmdsHdcp2Rx4SetRdMsgIDCode_WDINT(_HDCP22_AKE_SEND_H_PRIME);

        memcpy(g_pucHdcp2Sha256, (volatile BYTE xdata *)P63_35_HDCP_SHA_0, 32);

        // Load Km to Read SRAM
        ScalerTmdsHdcp2Rx4SetRdMsgValue_WDINT(0x01, g_pucHdcp2Sha256, 32);
    ...
    ScalerHdcp2Proc2_WDINT(g_pucTmdsHdcp2Rx4Km, pucTemp1);
    ScalerHdcp2Proc2_WDINT(g_pucTmdsHdcp2Rx4dKey0, pucTemp2);
    ScalerHdcp2Proc2_WDINT(g_pucTmdsHdcp2Rx4dKey1, pucTemp3);
