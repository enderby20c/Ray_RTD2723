...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerDpHdcp14Rx1.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
volatile bit g_bDpHdcp14Rx1AksvWritten = 0;
volatile WORD g_usDpHdcp14Rx1KsvFifoOffset;
StructDPMacRxHDCPAuthInfo g_stDpHdcp14Rx1AuthInfo;
#endif
BYTE g_ucDpHdcp14Rx1Caps;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx1ResetProc(void)
{
#if(_DP_HDCP14_RX1_REPEATER_SUPPORT == _ON)
...
#endif
    {
        ...
        CLR_DP_HDCP14_RX1_REPEATER_AUTH_1_START();
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx1RepeaterHandler(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX1);
...
#endif
    {
        ...
        if(GET_DP_HDCP14_RX1_AUTH_AKSV_WRITTEN() == _TRUE)
        {
            CLR_DP_HDCP14_RX1_AUTH_AKSV_WRITTEN();
            CLR_DP_HDCP14_RX1_AUTH_CPIRQ_TIMRE();
            SET_DP_HDCP14_RX1_REPEATER_AUTH_1_START();
        ...
        if(GET_DP_RX_HDCP_UPSTREAM_PORT() == _HDCP_D1_PORT)
        {
            ...
            {
                if((ScalerDpMacDphyRxHdcpCheckValid(enumInputPort) == _TRUE) && (GET_DP_HDCP14_RX1_AUTH_CPIRQ_TIMRE() == _FALSE))
                {
                    if(ScalerDpMacDphyRxHdcpCheckLvp(enumInputPort) == _FALSE)
                    {
                        SET_DP_HDCP14_RX1_AUTH_CPIRQ_TIMRE();
    ...
    switch(GET_DP_HDCP14_RX1_AUTH_STATE())
    {
        case _DP_RX_HDCP_STATE_IDLE:
            ...
            if(GET_DP_HDCP14_RX1_AUTH_STATE_CHANGE() == _TRUE)
            {
                ...
                CLR_DP_HDCP14_RX1_AUTH_STATE_CHANGE();
                CLR_DP_HDCP14_RX1_AUTH_REPEATER_PROC_TIMEOUT();
                CLR_DP_HDCP14_RX1_AUTH_CPIRQ_TIMRE();
                ...
                ScalerDpHdcp14Rx1EnableReadIrq(_DISABLE);

                ScalerDpHdcp14TxResetKsvBackup();

                SET_DP_HDCP14_RX1_UPSTREAM_EVENT(_DP_RX_HDCP_UPSTREAM_AUTH_EVENT_STOP_ENCRYPTION);
            }
...
#endif
            {
                ...
                {
                    if(GET_DP_HDCP14_RX1_REPEATER_AUTH_1_START() == _TRUE)
                    {
                        CLR_DP_HDCP14_RX1_REPEATER_AUTH_1_START();

                        ScalerDpHdcp14Rx1ChangeAuthState(_DP_RX_HDCP_STATE_AUTH_1);
                    }
                }
            }
        ...
        case _DP_RX_HDCP_STATE_AUTH_1:
            ...
            if(GET_DP_HDCP14_RX1_AUTH_STATE_CHANGE() == _TRUE)
            {
                ...
                if(GET_DP_RX_HDCP_UPSTREAM_PORT() == _HDCP_D1_PORT)
                {
                    ...
                    {
                        ScalerDpHdcp14Rx1ChangeAuthState(_DP_RX_HDCP_STATE_AUTH_2);
                    }
                }
                else
                {
                    ScalerDpHdcp14Rx1ChangeAuthState(_DP_RX_HDCP_STATE_AUTH_FAKE);
                }
            }
            ...
            else if(GET_DP_HDCP14_RX1_AUTH_REPEATER_PROC_TIMEOUT() == _TRUE)
            {
                CLR_DP_HDCP14_RX1_AUTH_REPEATER_PROC_TIMEOUT();
                ...
                ScalerDpHdcp14Rx1ChangeAuthState(_DP_RX_HDCP_STATE_AUTH_DONE);
            }
        ...
        case _DP_RX_HDCP_STATE_AUTH_2:
            ...
            if(ScalerDpHdcp14Rx1CheckBinfo() == _TRUE)
            {
                // Load KSV List to RX KSV FIFO
                ScalerDpHdcp14Rx1LoadKsvFifo();
                ...
                ScalerDpHdcp14Rx1SetKsvFifoFirst();

                // Enable 68xxx Read INT
                ScalerDpHdcp14Rx1EnableReadIrq(_ENABLE);

                ScalerTimerActiveTimerEvent(SEC(5), _SCALER_TIMER_EVENT_DP_RX1_HDCP_READ_IRQ_TIMEOUT);
            }

            if(GET_DP_HDCP14_RX1_AUTH_AKSV_WRITTEN() == _FALSE)
            {
        ...
        case _DP_RX_HDCP_STATE_AUTH_DONE:
            ...
            if(GET_DP_TX_HDCP_DOWNSTREAM_EVENT() == _DP_TX_HDCP_DOWNSTREAM_AUTH_EVENT_PASS)
            {
                ...
                {
                    if((ScalerDpHdcp14Rx1DeviceCountChange() == _TRUE) &&
                       ...
                       (ScalerDpMacDphyRxHdcpCheckLvp(enumInputPort) == _TRUE))
                    {
                        ...
                        ScalerDpHdcp14Rx1ReAuthProcess(enumInputPort);
                    }
                }
            }

            if(GET_DP_HDCP14_RX1_AUTH_READ_IRQ_TIMEOUT() == _TRUE)
            {
                ScalerDpHdcp14Rx1EnableReadIrq(_DISABLE);
            }

#if(_DP_MST_SUPPORT == _ON)
            ScalerDpHdcp14Rx1ChangeECFCheck();
#endif
            break;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx1ChangeECFCheck(void)
{
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx1ChangeAuthState(EnumDPRxHDCPAuthState enumHDCPAuthState)
{
    SET_DP_HDCP14_RX1_AUTH_STATE(enumHDCPAuthState);
    SET_DP_HDCP14_RX1_AUTH_STATE_CHANGE();
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx1EnableReadIrq(bit bEnable)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX1);
    ...
    else
    {
        ...
        CLR_DP_HDCP14_RX1_AUTH_READ_IRQ_TIMEOUT();
    }
}
...
// Output Value : None
//--------------------------------------------------
bit ScalerDpHdcp14Rx1CheckBinfo(void)
{
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx1LoadKsvFifo(void)
{
    WORD usIndex = 0;
    WORD usKsvOffset = 0;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx1SetKsvFifoFirst(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX1);
    ...
    for(g_usDpHdcp14Rx1KsvFifoOffset = 0; g_usDpHdcp14Rx1KsvFifoOffset < 15; g_usDpHdcp14Rx1KsvFifoOffset++)
    {
        if(GET_DP_TX_HDCP_KSV_FIFO_MAX() > g_usDpHdcp14Rx1KsvFifoOffset)
        {
            ...
            ScalerDpAuxRxSetDpcdValue(enumInputPort, 0x06, 0x80, (0x2C + g_usDpHdcp14Rx1KsvFifoOffset), g_pucDpMacTxHdcpKsvFifo[g_usDpHdcp14Rx1KsvFifoOffset]);
        }
        else
        {
            // DPCD [0x6802C ~ 0x6803A]
            ScalerDpAuxRxSetDpcdValue(enumInputPort, 0x06, 0x80, (0x2C + g_usDpHdcp14Rx1KsvFifoOffset), 0x00);
        }
    }
...
//--------------------------------------------------
bit ScalerDpHdcp14Rx1DeviceCountChange(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX1);
    BYTE ucBInfoDevice = 0;
    BYTE ucBInfoDepth = 0;
...
//--------------------------------------------------
// Description  : Change DP HDCP1.4 Repeater Capable for Rx1 Base On DPTX Clone Status
// Input Value  : InputPort, PortType, Clone Mode or not
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx1RepeaterSupportProc(void)
{
    bit bDpRx1HdcpSetRepeater = _OFF;
    EnumInputPort enumHdcpInputPort = ScalerDpHdcpRxRxDxMapping(_RX1);

    if(GET_DP_HDCP14_RX1_CAPABLE_SUPPORT() == _FALSE)
    {
        return;
    }
    ...
    if(GET_DP_MAC_TX_DOWNSTREAM_DPPLUG_STATUS() != _DP_UNPLUG)
    {
        if(GET_DP_RX_HDCP_UPSTREAM_PORT() == _HDCP_D1_PORT)
        {
            bDpRx1HdcpSetRepeater = _ON;
        }
    }

    if(bDpRx1HdcpSetRepeater != GET_DP_HDCP14_RX1_REPEATER_SUPPORT())
    {
        DebugMessageHDCP2("[D1]Hdcp1.4 Repeater Bit Changed", bDpRx1HdcpSetRepeater);

        if(bDpRx1HdcpSetRepeater == _ON)
        {
            SET_DP_HDCP14_RX1_REPEATER_SUPPORT();
        }
        else
        {
            CLR_DP_HDCP14_RX1_REPEATER_SUPPORT();
        }
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx1ReAuthProcess(EnumInputPort enumInputPort)
{
    ...
    if((ScalerDpAuxRxGetDpcdBitInfo(enumInputPort, 0x00, 0x01, 0x11, (_BIT1 | _BIT0)) == 0x00) &&
       (GET_DP_HDCP14_RX1_AUTH_STATE() != _DP_RX_HDCP_STATE_IDLE) &&
       (ScalerDpMacDphyRxHdcpCheckValid(enumInputPort) == _TRUE))
    {
