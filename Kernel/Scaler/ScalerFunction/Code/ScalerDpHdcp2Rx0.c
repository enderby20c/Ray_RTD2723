...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerDpHdcp2Rx0.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------

#include "ScalerFunctionInclude.h"
#include "DpHdcp2Rx0/ScalerDpHdcp2Rx0.h"
...
//----------------------------------------------------------------------------
volatile BYTE g_ucDpHdcp2Rx0Ctrl0;
volatile BYTE g_ucDpHdcp2Rx0Ctrl1;
BYTE g_pucDpHdcp2Rx0dKey0[16];
BYTE g_pucDpHdcp2Rx0dKey1[16];
BYTE g_pucDpHdcp2Rx0Km[16];
BYTE g_pucDpHdcp2Rx0Rtx[8];
BYTE g_pucDpHdcp2Rx0TxCaps[3];
BYTE g_pucDpHdcp2Rx0Rn[8];
BYTE g_pucDpHdcp2Rx0Rrx[8];
BYTE g_pucDpHdcp2Rx0VPrime[20];
BYTE g_pucDpHdcp2Rx0Caps[3];

#if(_DP_TX_SUPPORT == _ON)
volatile StructDpHdcp2RxAuthInfo g_stDpHdcp2Rx0AuthInfo;
#endif
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0Handler(void)
{
    BYTE pucTemp1[16] = {0};
    BYTE pucTemp2[16] = {0};
    BYTE pucTemp3[16] = {0};
    ...
    if(GET_DP_HDCP2_RX0_AKE_INIT_RECEIVED() == _TRUE)
    {
        CLR_DP_HDCP2_RX0_AKE_INIT_RECEIVED();

#if(_DP_TX_SUPPORT == _ON)
        if(GET_DP_HDCP2_RX0_REPEATER_SUPPORT() == _TRUE)
        {
            SET_DP_HDCP2_RX0_REPEATER_AUTH_1_START();
        }

        // Clear DPCD Repeater Info
        ScalerDpHdcp2Rx0ClearDpcdRepeaterInfo();
#endif
    ...
    if(GET_DP_HDCP2_RX0_AKE_NO_STORED_KM_RECEIVED() == _TRUE)
    {
        if(ScalerDpHdcp2Rx0CaptureKpubKm() == _TRUE)
        {
            // Decrypt Km by FW to Get 128 bit Km
            if(ScalerHdcp2RxRsaOaepCalculate(pucTemp1) == _TRUE)
            {
                ScalerHdcp2DkeyCalculate(_HDCP_2_2_DKEY_0, g_pucDpHdcp2Rx0Rtx, g_pucDpHdcp2Rx0Rrx, pucTemp1, g_pucDpHdcp2Rx0Rn);

                memcpy(pucTemp2, (volatile BYTE xdata *)P63_81_HDCP_AES_OUT_15, 16);

                ScalerHdcp2DkeyCalculate(_HDCP_2_2_DKEY_1, g_pucDpHdcp2Rx0Rtx, g_pucDpHdcp2Rx0Rrx, pucTemp1, g_pucDpHdcp2Rx0Rn);

                memcpy(pucTemp3, (volatile BYTE xdata *)P63_81_HDCP_AES_OUT_15, 16);

                ScalerHdcp2Proc2(g_pucDpHdcp2Rx0Km, pucTemp1);
                ScalerHdcp2Proc2(g_pucDpHdcp2Rx0dKey0, pucTemp2);
                ScalerHdcp2Proc2(g_pucDpHdcp2Rx0dKey1, pucTemp3);

                if(ScalerHdcp2HPrimeCalculate(pucTemp2, pucTemp3, g_pucDpHdcp2Rx0Rtx, g_pucDpHdcp2Rx0TxCaps, g_pucDpHdcp2Rx0Caps) == _TRUE)
                {
                    ScalerDpHdcp2Rx0CalculateKmKh();
                }
            }
        }
    }

    if(GET_DP_HDCP2_RX0_AKE_STORED_KM_RECEIVED() == _TRUE)
    {
        CLR_DP_HDCP2_RX0_AKE_STORED_KM_RECEIVED();

        DebugMessageHDCP2("[D0]AKE_STORED_KM", 0);
    }

    if(GET_DP_HDCP2_RX0_LC_INIT_RECEIVED() == _TRUE)
    {
        CLR_DP_HDCP2_RX0_LC_INIT_RECEIVED();

        DebugMessageHDCP2("[D0]LC_INIT", 0);
    }

    if(GET_DP_HDCP2_RX0_SKE_SEND_EKS_RECEIVED() == _TRUE)
    {
        CLR_DP_HDCP2_RX0_SKE_SEND_EKS_RECEIVED();

#if(_DP_TX_SUPPORT == _ON)
        if(GET_DP_HDCP2_RX0_REPEATER_SUPPORT() == _TRUE)
        {
            SET_DP_HDCP2_RX0_REPEATER_AUTH_2_START();
        }
#endif
    ...
    if(GET_DP_HDCP2_RX0_STREAM_MANAGE_RECEIVED() == _TRUE)
    {
        CLR_DP_HDCP2_RX0_STREAM_MANAGE_RECEIVED();

#if(_DP_TX_SUPPORT == _ON)
        SET_DP_HDCP2_RX0_CONTENT_STREAM_MANAGEMENT();
#endif
    }
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0ResetProc(void)
{
    if(GET_DP_HDCP2_RX0_AUTHENTICATION_DONE() == _TRUE)
    {
        ...
        ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX0_AKE_INIT);

        ScalerDpHdcp2Rx0ClearDpcdReadValue(_HDCP22_RX_H_PRIME, 32);
        ScalerDpHdcp2Rx0ClearDpcdReadValue(_HDCP22_RX_L_PRIME, 32);
        ScalerDpHdcp2Rx0ClearDpcdReadValue(_HDCP22_RX_M_PRIME, 32);
        ScalerDpHdcp2Rx0ClearDpcdReadValue(_HDCP22_RX_PAIRING_INFO, 16);

        CLR_DP_HDCP2_RX0_AKE_INIT_RECEIVED();
        CLR_DP_HDCP2_RX0_AKE_NO_STORED_KM_RECEIVED();
        CLR_DP_HDCP2_RX0_AKE_STORED_KM_RECEIVED();
        CLR_DP_HDCP2_RX0_LC_INIT_RECEIVED();
        CLR_DP_HDCP2_RX0_SKE_SEND_EKS_RECEIVED();
        CLR_DP_HDCP2_RX0_AUTHENTICATION_ACTIVE();
...
#if(_DP_TX_SUPPORT == _ON)
        CLR_DP_HDCP2_RX0_REPEATER_AUTH_1_START();
        CLR_DP_HDCP2_RX0_REPEATER_AUTH_2_START();

        ScalerDpHdcp2Rx0ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_IDLE);

        ScalerDpHdcp2Rx0ClearDpcdReadValue(_HDCP22_RX_V_PRIME, 190);
#endif
    }
}
...
//--------------------------------------------------
bit ScalerDpHdcp2Rx0CaptureKpubKm(void)
{
    BYTE ucIndex = 0x00;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0CalculateKmKh(void)
{
    BYTE ucIndex = 0x00;
    BYTE pucSHA256[32] = {0};
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX0);
    ...
    SET_DP_HDCP2_RX0_WAITING_H_PRIME_CPIRQ_ASSERTED();

    // Initialize Pairing Available CPIRQ Flag To Record Pairing Available CPIRQ Asserted Status
    CLR_DP_HDCP2_RX0_RECORD_PAIRING_INFO_CPIRQ_NO_ASSERTED();

    SET_DP_HDCP2_RX0_AKE_NO_STORED_KM_DONE();
    ...
    CLR_DP_HDCP2_RX0_WAITING_H_PRIME_CPIRQ_ASSERTED();

    // Assert Pairing Available CPIRQ If No Asserting in Pairing Info INT State
    if(GET_DP_HDCP2_RX0_RECORD_PAIRING_INFO_CPIRQ_NO_ASSERTED() == _TRUE)
    {
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0CancelTimerEvent(void)
{
    ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX0_H_PRIME);
    ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX0_SKE);

#if(_DP_TX_SUPPORT == _ON)
    CLR_DP_HDCP2_RX0_REAUTH_LONG_HPD();

    ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX0_M_PRIME);
    ScalerTimerWDCancelTimerEvent(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX0_AKE_INIT);
#endif
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0ClearDpcdReadValue(BYTE ucRSAType, WORD usLength)
{
    ...
    ScalerDpAuxRx0DpcdGroupReset(0x06, HIBYTE(usDpcdAddress), LOBYTE(usDpcdAddress), usLength);
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0RepeaterHandler(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX0);

    if(ScalerDpHdcpRxGetHdcpMode(enumInputPort) == _HDCP_22)
    {
        if(GET_DP_HDCP2_RX0_CAPABLE_SUPPORT() == _TRUE)
        {
            if(GET_DP_HDCP2_RX0_REPEATER_SUPPORT() == _TRUE)
            {
                ...
                if(GET_DP_RX_HDCP2_UPSTREAM_PORT() == _HDCP_D0_PORT)
                {
                    ...
                    {
                        if((ScalerDpMacDphyRxHdcpCheckValid(enumInputPort) == _TRUE) && (GET_DP_HDCP2_RX0_AUTH_CPIRQ_TIMRE() == _FALSE))
                        {
                            if(ScalerDpMacDphyRxHdcpCheckLvp(enumInputPort) == _FALSE)
                            {
                                SET_DP_HDCP2_RX0_AUTH_CPIRQ_TIMRE();
                    ...
                    if((ScalerDpHdcp2Rx0ReauthEventCheck() == _TRUE) && (GET_DP_HDCP2_RX0_AUTH_STATE() != _DP_RX_HDCP2_REPEATER_STATE_IDLE))
                    {
                        if(GET_DP_HDCP2_RX0_AUTH_STATE() != _DP_RX_HDCP2_REPEATER_STATE_AUTH_DONE)
                        {
                            ScalerDpHdcp2Rx0ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_AUTH_1);

                            SET_DP_HDCP2_RX0_REPEATER_DFP_REAUTH();
                        }
                        else
                        {
                            CLR_DP_HDCP2_RX0_REPEATER_DFP_REAUTH();

                            ScalerDpHdcp2Rx0ReAuthProcess(enumInputPort);
                        }
                    }
                }

                if((GET_DP_HDCP2_RX0_AUTH_STATE() != _DP_RX_HDCP2_REPEATER_STATE_IDLE) && (ScalerDpAuxRxGetLTStatus(enumInputPort) == _DP_LINK_TRAINING_NONE))
                {
                    ScalerDpHdcp2Rx0ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_IDLE);
                }
            }
    ...
    switch(GET_DP_HDCP2_RX0_AUTH_STATE())
    {
        case _DP_RX_HDCP2_REPEATER_STATE_IDLE:
            ...
            if(GET_DP_HDCP2_RX0_AUTH_STATE_CHANGE() == _TRUE)
            {
                ...
                CLR_DP_HDCP2_RX0_AUTH_STATE_CHANGE();
                CLR_DP_HDCP2_RX0_AUTH_REPEATER_PROC_TIMEOUT();
                CLR_DP_HDCP2_RX0_AUTH_CPIRQ_TIMRE();
                CLR_DP_HDCP2_RX0_CONTENT_STREAM_MANAGEMENT();
                CLR_DP_HDCP2_RX0_REPEATER_DFP_REAUTH();

                ScalerTimerCancelTimerEvent(_SCALER_TIMER_EVENT_DP_RX0_HDCP2_REPEATER_PROC_TIMEOUT);

                SET_DP_HDCP2_RX0_UPSTREAM_EVENT(_DP_RX_HDCP2_UPSTREAM_AUTH_EVENT_STOP_ENCRYPTION);
            }

            if((ScalerDpHdcpRxGetHdcpMode(enumInputPort) == _HDCP_22) && (GET_DP_HDCP2_RX0_REPEATER_SUPPORT() == _TRUE))
            {
                if(GET_DP_HDCP2_RX0_REPEATER_AUTH_1_START() == _TRUE)
                {
                    CLR_DP_HDCP2_RX0_REPEATER_AUTH_1_START();
        ...
        case _DP_RX_HDCP2_REPEATER_STATE_AUTH_1:
            ...
            if(GET_DP_HDCP2_RX0_AUTH_STATE_CHANGE() == _TRUE)
            {
                ...
                if(GET_DP_RX_HDCP2_UPSTREAM_PORT() == _HDCP_D0_PORT)
                {
                    ...
                    if((GET_DP_HDCP2_TX_CAPABLE_SUPPORTED() == _TRUE) && (GET_DP_HDCP2_TX_VERSION_SUPPORTED() == _TRUE))
                    {
                        ...
                        SET_DP_HDCP2_RX0_UPSTREAM_EVENT(_DP_RX_HDCP2_UPSTREAM_AUTH_EVENT_START_HANDSHAKE);
                    ...
                    else
                    {
                        ...
                        ScalerDpHdcp2Rx0ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_AUTH_2);
                    }
                }
                else
                {
                    ...
                    ScalerDpHdcp2Rx0ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_AUTH_FAKE);
                }
            }
            ...
            else if(GET_DP_HDCP2_RX0_AUTH_REPEATER_PROC_TIMEOUT() == _TRUE)
            {
                CLR_DP_HDCP2_RX0_AUTH_REPEATER_PROC_TIMEOUT();

                if((GET_DP_HDCP2_TX_CAPABLE_SUPPORTED() == _TRUE) && (GET_DP_HDCP2_TX_VERSION_SUPPORTED() == _TRUE))
                {
                    SET_DP_HDCP2_RX0_UPSTREAM_EVENT(_DP_RX_HDCP2_UPSTREAM_AUTH_EVENT_TIMEOUT);
                }
                else if(GET_DP_TX_HDCP_DOWNSTREAM_SUPPORTED() == _TRUE)
                {
                ...
                if(GET_DP_HDCP2_RX0_REPEATER_DFP_REAUTH() == _TRUE)
                {
                    CLR_DP_HDCP2_RX0_REPEATER_DFP_REAUTH();
                ...
                ScalerDpHdcp2Rx0ChangeAuthState(_DP_RX_HDCP2_REPEATER_STATE_AUTH_DONE);
            }
        ...
        case _DP_RX_HDCP2_REPEATER_STATE_AUTH_2:
            ...
            if(GET_DP_HDCP2_RX0_REPEATER_AUTH_2_START() == _TRUE)
            {
                CLR_DP_HDCP2_RX0_REPEATER_AUTH_2_START();

                if(ScalerDpHdcp2Rx0CheckRxInfo() == _TRUE)
                {
                    ScalerDpHdcp2Rx0LoadReceiverIdList();

                    ScalerDpHdcp2Rx0VComputeProc();
                }
        ...
        case _DP_RX_HDCP2_REPEATER_STATE_AUTH_FAKE:
            ...
            if(GET_DP_HDCP2_RX0_REPEATER_AUTH_2_START() == _TRUE)
            {
                ...
                // Fake Authentication
                ScalerDpHdcp2Rx0FakeAuthenProc();
        ...
        case _DP_RX_HDCP2_REPEATER_STATE_AUTH_DONE:
            ...
            ScalerDpHdcp2Rx0ChangeECFCheck();
#endif
            break;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0ChangeECFCheck(void)
{
    ...
    if(bECFChanged == _TRUE)
    {
        if((GET_DP_HDCP2_TX_CAPABLE_SUPPORTED() == _TRUE) && (GET_DP_HDCP2_TX_VERSION_SUPPORTED() == _TRUE) && (GET_DP_TX_STREAM_SOURCE() == _DP_TX_SOURCE_MST))
        {
            ...
            SET_DP_HDCP2_RX0_UPSTREAM_EVENT(_DP_RX_HDCP2_UPSTREAM_AUTH_EVENT_ECF_CHANGE);
        }
        else
        {
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0ChangeAuthState(EnumDpHdcp2RxRepeaterAuthState enumHdcp2AuthState)
{
    SET_DP_HDCP2_RX0_AUTH_STATE(enumHdcp2AuthState);
    SET_DP_HDCP2_RX0_AUTH_STATE_CHANGE();
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0SetRepeaterProcTimeout(void)
{
    SET_DP_HDCP2_RX0_AUTH_REPEATER_PROC_TIMEOUT();
}
...
// Output Value : None
//--------------------------------------------------
bit ScalerDpHdcp2Rx0CheckRxInfo(void)
{
    BYTE pucRxInfo[2] = {0};
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0LoadReceiverIdList(void)
{
    BYTE ucIndex = 0x00;
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX0);
    EnumDpAuxRxModeStatus enumDpAuxRxModeStatus = GET_DP_AUX_RX0_MODE();
...
// Output Value : None
//--------------------------------------------------
bit ScalerDpHdcp2Rx0VComputeProc(void)
{
    BYTE ucIndex = 0x00;
    BYTE pucRxInfo[2] = {0};
    BYTE pucSeqNumV[3] = {0};
    ...
    {
        g_pucDpHdcp2Rx0VPrime[ucIndex] = ScalerGetByte(P63_35_HDCP_SHA_0 + ucIndex);

        // DPCD [0x69335 ~ 0x69344]
        ScalerDpAuxRxSetDpcdValue(enumInputPort, 0x06, 0x93, (0x35 + ucIndex), g_pucDpHdcp2Rx0VPrime[ucIndex]);
    }
...
// Output Value : None
//--------------------------------------------------
bit ScalerDpHdcp2Rx0FakeAuthenProc(void)
{
    BYTE ucIndex = 0x00;
    BYTE pucRxInfo[2] = {0};
    BYTE pucSeqNumV[3] = {0};
    ...
    ScalerDpAuxRx0DpcdGroupReset(0x06, 0x93, 0x4A, 150);
...
void ScalerDpHdcp2Rx0RepeaterSupportProc(void)
{
    ...
    if(bDPRx0Hdcp2SetRepeater != GET_DP_HDCP2_RX0_REPEATER_SUPPORT())
    {
        if(bDPRx0Hdcp2SetRepeater == _ON)
        {
            SET_DP_HDCP2_RX0_REPEATER_SUPPORT();
        }
        else
        {
            CLR_DP_HDCP2_RX0_REPEATER_SUPPORT();
        }
...
// Output Value : None
//--------------------------------------------------
bit ScalerDpHdcp2Rx0ReauthEventCheck(void)
{
    BYTE ucReauth = 0;
    ...
    if((GET_DP_HDCP2_TX_CAPABLE_SUPPORTED() == _TRUE) && (GET_DP_HDCP2_TX_VERSION_SUPPORTED() == _TRUE))
    {
        ...
        {
            DebugMessageHDCP2("DPRX0 HDCP2 - Get DPTX HDCP2 ReAuth Event", 0);
    ...
    else
    {
        if(GET_DP_TX_HDCP_DOWNSTREAM_SUPPORTED() == _TRUE)
        {
            ...
            {
                DebugMessageHDCP2("DPRX0 HDCP2 - Get DPTX HDCP14 ReAuth Event", 0);
...
//--------------------------------------------------
bit ScalerDpHdcp2Rx0ReAuthStatusCheck(void)
{
    if(GET_DP_HDCP2_RX0_REAUTH_LONG_HPD() == _TRUE)
    {
        return _TRUE;
    }
    else
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0ReAuthProcess(EnumInputPort enumInputPort)
{
    ...
    if((ScalerDpAuxRxGetDpcdBitInfo(enumInputPort, 0x00, 0x01, 0x11, (_BIT1 | _BIT0)) == 0x00) &&
       (GET_DP_HDCP2_RX0_AUTH_STATE() != _DP_RX_HDCP2_REPEATER_STATE_IDLE) &&
       (ScalerDpMacDphyRxHdcpCheckValid(enumInputPort) == _TRUE))
    {
        SET_DP_HDCP2_RX0_REAUTH_LONG_HPD();

        ScalerTimerReactiveTimerEvent(SEC(4), _SCALER_TIMER_EVENT_DP_RX0_HDCP2_LONG_HOTPLUG_EVENT);
    }
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx0ClearDpcdRepeaterInfo(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX0);
