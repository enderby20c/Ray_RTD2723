...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerTypeC1Cc_WDINT.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
//****************************************************************************
//--------------------------------------------------
// Description  : WD Timer Event Proc of CC RX1
// Input Value  : enumEventID, pucActiveWDID, pucActiveWDTime
// Output Value : None
//--------------------------------------------------
void ScalerTypeC1CcWDTimerEventProc_WDINT(EnumScalerWDTimerEventID enumEventID, BYTE *pucActiveWDID, BYTE *pucActiveWDTime)
{
    ...
    switch(enumEventID)
    {
        case _SCALER_WD_TIMER_EVENT_CC_1_DETECT_VBUS:
            ...
            ScalerTypeCAdcOnOffCompIntSetting_WDINT(_TYPE_C_CHANNEL_1_VMON, _ON, GET_TYPE_C_1_5V_LOW_BND());

            // Enable [adc1_comp_vbus_on_en], and Wait ADC VbusSafe5V [adc1_comp_vbus_on] INT Triggered, then Set HW FSM to Attached.SNK
            ScalerTypeCAdcOnOffCompIntControl_WDINT(_TYPE_C_CHANNEL_1_VMON, _ON, _ENABLE);
        ...
        case _SCALER_WD_TIMER_EVENT_CC_1_CHECK_VBUS_0V:
            ...
            if(ScalerTypeCAdcCheckVbusSafe0V_WDINT(_EMB_TYPE_C_1) == _TRUE)
            {
                ScalerTypeC1PmicVbusDischarge_WDINT(_OFF);
            }
            else
            {
                *pucActiveWDID = _SCALER_WD_TIMER_EVENT_CC_1_CHECK_VBUS_0V;
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC1CcVconnControl_WDINT(bit bAction)
{
    if(bAction == _ON)
    {
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC1CcVbusDetCtrl_WDINT(bit bAction)
{
...
// Output Value : None
//--------------------------------------------------
BYTE ScalerTypeC1CcDetResistor_WDINT(void)
{
    // Only Update Resistor of CC Pin, Not Vconn Pin (Flip : [7:4] CC1, Un-Flip : [3:0] CC2)
    if(GET_TYPE_C_1_CC_ORIENTATION() == _TYPE_C_ORIENTATION_UNFLIP)
    {
        if(ScalerGetBit_WDINT(P6C_2A_CC_ANA_DET_R, (_BIT7 | _BIT6 | _BIT5 | _BIT4)) == 0xF0)
        {
    ...
    {
        if(ScalerGetBit_WDINT(P6C_2A_CC_ANA_DET_R, (_BIT3 | _BIT2 | _BIT1 | _BIT0)) == 0x0F)
        {
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC1CcSwitchPowerRole_WDINT(BYTE ucTargetRole)
{
    if(ucTargetRole == _PD_POWER_SRC)
    {
        // 1. Set Rp to Default CC Rp
#if(_EMB_TYPE_C_1_SRC_DEF_CUR == _TYPE_C_SRC_DEF_USB)
        ScalerSetBit_WDINT(P6C_37_CC_HW_Z_CFG, ~(_BIT7 | _BIT6), (_TYPE_C_SET_RP_DEF << 6));
#elif(_EMB_TYPE_C_1_SRC_DEF_CUR == _TYPE_C_SRC_DEF_1P5A)
        ScalerSetBit_WDINT(P6C_37_CC_HW_Z_CFG, ~(_BIT7 | _BIT6), (_TYPE_C_SET_RP_1P5A << 6));
#else
        ScalerSetBit_WDINT(P6C_37_CC_HW_Z_CFG, ~(_BIT7 | _BIT6), (_TYPE_C_SET_RP_3P0A << 6));
#endif

        // 2. Switch Type-C HW State From SNK to SRC
        ScalerSetBit_WDINT(P6C_34_CC_HW_STATE_CTRL, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3), _BIT6);
        ...
        if((GET_TYPE_C_1_CC_PORT_ROLE() == _TYPE_C_PORT_DRP) && (GET_TYPE_C_1_CC_DRP_PREFER() == _TYPE_C_DRP_TRY_SRC))
        {
            ...
            ScalerSetBit_WDINT(P6C_30_CC_HW_MODE_CTRL, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT1));

            // [0x6C58] : [1] hw_try_wait_snk_irq, [0] hw_try_wait_snk_irq_en
            // Clear and Enable hw_try_wait_snk_irq
            ScalerSetBit_WDINT(P6C_58_DBCC_SNK_DET_COND, ~(_BIT3 | _BIT1 | _BIT0), (_BIT1 | _BIT0));
    ...
    else
    {
        ...
        ScalerSetBit_WDINT(P6C_34_CC_HW_STATE_CTRL, ~(_BIT6 | _BIT5 | _BIT4 | _BIT3), _BIT5);
        ...
        if((GET_TYPE_C_1_CC_PORT_ROLE() == _TYPE_C_PORT_DRP) && (GET_TYPE_C_1_CC_DRP_PREFER() == _TYPE_C_DRP_TRY_SRC))
        {
            ...
            ScalerSetBit_WDINT(P6C_30_CC_HW_MODE_CTRL, ~(_BIT3 | _BIT2 | _BIT1 | _BIT0), (_BIT3 | _BIT1 | _BIT0));

            // [0x6C58] : [1] hw_try_wait_snk_irq, [0] hw_try_wait_snk_irq_en
            // Clear and Disable hw_try_wait_snk_irq
            ScalerSetBit_WDINT(P6C_58_DBCC_SNK_DET_COND, ~(_BIT3 | _BIT1 | _BIT0), _BIT1);
...
// Output Value : None
//--------------------------------------------------
void ScalerTypeC1CcToolWarningUIControl_WDINT(bit bAction)
{
