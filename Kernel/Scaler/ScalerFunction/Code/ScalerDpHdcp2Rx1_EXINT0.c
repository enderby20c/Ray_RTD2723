...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerDpHdcp2Rx1_EXINT0.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx1Handler_EXINT0(void)
{
    BYTE ucMessageId = 0;
    BYTE ucIndex = 0;
    BYTE pucTemp1[16] = {0};
    ...
    if(ScalerDpAuxRxGetGlobalIrq_EXINT0(enumInputPort) == _TRUE)
    {
        ...
        switch(ucMessageId)
        {
            case _HDCP22_AKE_INIT:
                ...
                // HDCP2 Reset Proc
                ScalerDpHdcp2Rx1ResetProc_EXINT0();

                // Save Transmitter Rtx to FW (MSB located at index 0)
                ScalerDpHdcp2Rx1LoadDpcdValue_EXINT0(0x06, 0x90, 0x00, 8, g_pucDpHdcp2Rx1Rtx);

                // Save TxCaps to FW (MSB located at index 0)
                ScalerDpHdcp2Rx1LoadDpcdValue_EXINT0(0x06, 0x90, 0x08, 3, g_pucDpHdcp2Rx1TxCaps);

                // Generate the psudo-random key of Rrx
                ScalerHdcp2RxGenRrx_EXINT0(g_pucDpHdcp2Rx1Rrx);

                // Load Rrx to DPCD Table (MSB located at index 0)
                ScalerDpHdcp2Rx1SetDpcdReadValue_EXINT0(_HDCP22_RRX, g_pucDpHdcp2Rx1Rrx, 8);

                ScalerDpHdcp2Rx1CancelTimerEvent_EXINT0();

                ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_AKE_INIT);

                // Execute AKE_INIT Procedure after 3msec
                ScalerTimerWDActivateTimerEvent_EXINT0(3, _SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_AKE_INIT);

                SET_DP_HDCP2_RX1_AKE_INIT_RECEIVED();

#if(_DP_HDCP_POWER_OFF_HANDSHAKE_SUPPORT == _ON)
                SET_DP_HDCP2_RX1_HANDSHAKE_START();
#endif
            ...
            case _HDCP22_AKE_NO_STORED_KM:
                ...
                SET_DP_HDCP2_RX1_AKE_NO_STORED_KM_RECEIVED();
                SET_DP_HDCP2_RX1_AUTHENTICATION_ACTIVE();

                // If not clear (H' & Pairing Info) in WDINT in time, cancel the WD and clear it here;
                if(GET_DP_HDCP2_RX1_CLEAR_DPCD_READ_VALUE_DONE() == _FALSE)
                {
                    ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_AKE_INIT);

                    ScalerDpHdcp2Rx1ClearDpcdReadValue_EXINT0(_HDCP22_RX_H_PRIME, 32);
                    ScalerDpHdcp2Rx1ClearDpcdReadValue_EXINT0(_HDCP22_RX_PAIRING_INFO, 16);
                }
            ...
            case _HDCP22_AKE_SEND_PAIRING_INFO:
                ...
                for(BYTE ucLoopIndex = 0; ucLoopIndex < 16; ucLoopIndex++)
                {
                    if(ucLoopIndex < 8)
                    {
                        g_pucHdcp2M[ucLoopIndex] = g_pucDpHdcp2Rx1Rtx[ucLoopIndex];
                    }
                    else
                    {
                        g_pucHdcp2M[ucLoopIndex] = g_pucDpHdcp2Rx1Rrx[ucLoopIndex - 8];
                    }
                }
                ...
                ScalerHdcp2Proc2_EXINT0(pucTemp1, g_pucDpHdcp2Rx1Km);
                ...
                if(GET_DP_HDCP2_RX1_WAITING_H_PRIME_CPIRQ_ASSERTED() == _FALSE)
                {
                ...
                else
                {
                    ...
                    SET_DP_HDCP2_RX1_RECORD_PAIRING_INFO_CPIRQ_NO_ASSERTED();
                }
            ...
            case _HDCP22_AKE_STORED_KM:
                ...
                ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_H_PRIME);

                // Execute H' Procedure after 4msec
                ScalerTimerWDActivateTimerEvent_EXINT0(4, _SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_H_PRIME);
            ...
            case _HDCP22_LC_INIT:
                ...
                ScalerDpHdcp2Rx1LoadDpcdValue_EXINT0(0x06, 0x92, 0xF0, 8, g_pucDpHdcp2Rx1Rn);

                ScalerHdcp2Proc2_EXINT0(pucTemp2, g_pucDpHdcp2Rx1dKey0);
                ScalerHdcp2Proc2_EXINT0(pucTemp3, g_pucDpHdcp2Rx1dKey1);
                ...
                if(GET_DP_HDCP2_RX1_CLEAR_DPCD_READ_VALUE_DONE() == _FALSE)
                {
                    ...
                    ScalerDpHdcp2Rx1ClearDpcdReadValue_EXINT0(_HDCP22_RX_L_PRIME, 32);
                }

                if(ScalerHdcp2LPrimeCalculate_EXINT0(pucTemp2, pucTemp3, g_pucDpHdcp2Rx1Rrx, g_pucDpHdcp2Rx1Rn) == _TRUE)
                {
                    ...
                    ScalerDpHdcp2Rx1SetDpcdReadValue_EXINT0(_HDCP22_RX_L_PRIME, g_pucHdcp2Sha256, 32);
                }

                SET_DP_HDCP2_RX1_LC_INIT_RECEIVED();

                break;

            case _HDCP22_SKE_SEND_EKS:

                ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_SKE);

                // Execute Ske_Send_Eks Procedure after 4msec
                ScalerTimerWDActivateTimerEvent_EXINT0(4, _SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_SKE);
            ...
            case _HDCP22_REPEATERAUTH_STREAM_MANAGE:
                ...
                ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_M_PRIME);
                ...
                ScalerTimerWDActivateTimerEvent_EXINT0(4, _SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_M_PRIME);
                ...
                if(GET_DP_HDCP2_RX1_CLEAR_DPCD_READ_VALUE_DONE() == _FALSE)
                {
                    ...
                    ScalerDpHdcp2Rx1ClearDpcdReadValue_EXINT0(_HDCP22_RX_M_PRIME, 32);
                }
#endif
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx1LoadDpcdValue_EXINT0(BYTE ucDpcdAccessPortH, BYTE ucDpcdAccessPortM, BYTE ucDpcdAccessPortL, BYTE ucLength, BYTE *pucReadArray)
{
    BYTE ucIndex = 0x00;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx1ResetProc_EXINT0(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping_EXINT0(_RX1);
    ...
    CLR_DP_HDCP2_RX1_CLEAR_DPCD_READ_VALUE_DONE();
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx1SetDpcdReadValue_EXINT0(BYTE ucRSAType, BYTE *pucReadArray, WORD usLength)
{
    BYTE ucIndex = 0x00;
    WORD usDpcdAddress = 0;
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping_EXINT0(_RX1);
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx1ClearDpcdReadValue_EXINT0(BYTE ucRSAType, WORD usLength)
{
    WORD usDpcdAddress = 0;
    ...
    ScalerDpAuxRx1DpcdGroupReset_EXINT0(0x06, HIBYTE(usDpcdAddress), LOBYTE(usDpcdAddress), usLength);
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp2Rx1CancelTimerEvent_EXINT0(void)
{
    ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_H_PRIME);
    ScalerTimerWDCancelTimerEvent_EXINT0(_SCALER_WD_TIMER_EVENT_DP_HDCP2_RX1_SKE);

#if(_DP_TX_SUPPORT == _ON)
    CLR_DP_HDCP2_RX1_REAUTH_LONG_HPD_EXINT();
