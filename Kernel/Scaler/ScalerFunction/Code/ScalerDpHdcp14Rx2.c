...
//----------------------------------------------------------------------------------------------------
// ID Code      : ScalerDpHdcp14Rx2.c No.0000
// Update Note  :
//----------------------------------------------------------------------------------------------------
...
volatile bit g_bDpHdcp14Rx2AksvWritten = 0;
volatile WORD g_usDpHdcp14Rx2KsvFifoOffset;
StructDPMacRxHDCPAuthInfo g_stDpHdcp14Rx2AuthInfo;
#endif
BYTE g_ucDpHdcp14Rx2Caps;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx2ResetProc(void)
{
#if(_DP_HDCP14_RX2_REPEATER_SUPPORT == _ON)
...
#endif
    {
        ...
        CLR_DP_HDCP14_RX2_REPEATER_AUTH_1_START();
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx2RepeaterHandler(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);
...
#endif
    {
        ...
        if(GET_DP_HDCP14_RX2_AUTH_AKSV_WRITTEN() == _TRUE)
        {
            CLR_DP_HDCP14_RX2_AUTH_AKSV_WRITTEN();
            CLR_DP_HDCP14_RX2_AUTH_CPIRQ_TIMRE();
            SET_DP_HDCP14_RX2_REPEATER_AUTH_1_START();
        ...
        if(GET_DP_RX_HDCP_UPSTREAM_PORT() == _HDCP_D2_PORT)
        {
            ...
            {
                if((ScalerDpMacDphyRxHdcpCheckValid(enumInputPort) == _TRUE) && (GET_DP_HDCP14_RX2_AUTH_CPIRQ_TIMRE() == _FALSE))
                {
                    if(ScalerDpMacDphyRxHdcpCheckLvp(enumInputPort) == _FALSE)
                    {
                        SET_DP_HDCP14_RX2_AUTH_CPIRQ_TIMRE();

                        ScalerDpMacDphyRxCpIRQ(enumInputPort, _DP_HDCP_BSTATUS_LINK_INTEGRITY_FAIL);

                        ScalerTimerActiveTimerEvent(SEC(0.02), _SCALER_TIMER_EVENT_DP_RX2_HDCP_CPIRQ_EVENT);
                    }
                }
            }
        }
    ...
    switch(GET_DP_HDCP14_RX2_AUTH_STATE())
    {
        case _DP_RX_HDCP_STATE_IDLE:
            ...
            if(GET_DP_HDCP14_RX2_AUTH_STATE_CHANGE() == _TRUE)
            {
                ...
                CLR_DP_HDCP14_RX2_AUTH_STATE_CHANGE();
                CLR_DP_HDCP14_RX2_AUTH_REPEATER_PROC_TIMEOUT();
                CLR_DP_HDCP14_RX2_AUTH_CPIRQ_TIMRE();
                ...
                ScalerDpHdcp14Rx2EnableReadIrq(_DISABLE);

                ScalerDpHdcp14TxResetKsvBackup();

                SET_DP_HDCP14_RX2_UPSTREAM_EVENT(_DP_RX_HDCP_UPSTREAM_AUTH_EVENT_STOP_ENCRYPTION);
            }
...
#endif
            {
                ...
                {
                    if(GET_DP_HDCP14_RX2_REPEATER_AUTH_1_START() == _TRUE)
                    {
                        CLR_DP_HDCP14_RX2_REPEATER_AUTH_1_START();

                        ScalerDpHdcp14Rx2ChangeAuthState(_DP_RX_HDCP_STATE_AUTH_1);
                    }
                }
            }
        ...
        case _DP_RX_HDCP_STATE_AUTH_1:
            ...
            if(GET_DP_HDCP14_RX2_AUTH_STATE_CHANGE() == _TRUE)
            {
                ...
                if(GET_DP_RX_HDCP_UPSTREAM_PORT() == _HDCP_D2_PORT)
                {
                    ...
                    if(GET_DP_TX_HDCP_DOWNSTREAM_SUPPORTED() == _TRUE)
                    {
                        ...
                        ScalerTimerActiveTimerEvent(SEC(6), _SCALER_TIMER_EVENT_DP_RX2_HDCP_REPEATER_PROC_TIMEOUT);
                    }
                    else
                    {
                        ScalerDpHdcp14Rx2ChangeAuthState(_DP_RX_HDCP_STATE_AUTH_2);
                    }
                }
                else
                {
                    ScalerDpHdcp14Rx2ChangeAuthState(_DP_RX_HDCP_STATE_AUTH_FAKE);
                }
            }
            ...
            else if(GET_DP_HDCP14_RX2_AUTH_REPEATER_PROC_TIMEOUT() == _TRUE)
            {
                CLR_DP_HDCP14_RX2_AUTH_REPEATER_PROC_TIMEOUT();
                ...
                ScalerDpHdcp14Rx2ChangeAuthState(_DP_RX_HDCP_STATE_AUTH_DONE);
            }
        ...
        case _DP_RX_HDCP_STATE_AUTH_2:
            ...
            if(ScalerDpHdcp14Rx2CheckBinfo() == _TRUE)
            {
                // Load KSV List to RX KSV FIFO
                ScalerDpHdcp14Rx2LoadKsvFifo();
                ...
                ScalerDpHdcp14Rx2SetKsvFifoFirst();

                // Enable 68xxx Read INT
                ScalerDpHdcp14Rx2EnableReadIrq(_ENABLE);

                ScalerTimerActiveTimerEvent(SEC(5), _SCALER_TIMER_EVENT_DP_RX2_HDCP_READ_IRQ_TIMEOUT);
            }

            if(GET_DP_HDCP14_RX2_AUTH_AKSV_WRITTEN() == _FALSE)
            {
        ...
        case _DP_RX_HDCP_STATE_AUTH_DONE:
            ...
            if(GET_DP_TX_HDCP_DOWNSTREAM_EVENT() == _DP_TX_HDCP_DOWNSTREAM_AUTH_EVENT_PASS)
            {
                ...
                {
                    if((ScalerDpHdcp14Rx2DeviceCountChange() == _TRUE) &&
                       ...
                       (ScalerDpMacDphyRxHdcpCheckLvp(enumInputPort) == _TRUE))
                    {
                        ...
                        ScalerDpHdcp14Rx2ReAuthProcess(enumInputPort);
                    }
                }
            }

            if(GET_DP_HDCP14_RX2_AUTH_READ_IRQ_TIMEOUT() == _TRUE)
            {
                ScalerDpHdcp14Rx2EnableReadIrq(_DISABLE);
            }

#if(_DP_MST_SUPPORT == _ON)
            ScalerDpHdcp14Rx2ChangeECFCheck();
#endif
            break;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx2ChangeECFCheck(void)
{
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx2ChangeAuthState(EnumDPRxHDCPAuthState enumHDCPAuthState)
{
    SET_DP_HDCP14_RX2_AUTH_STATE(enumHDCPAuthState);
    SET_DP_HDCP14_RX2_AUTH_STATE_CHANGE();
}
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx2EnableReadIrq(bit bEnable)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);
    ...
    else
    {
        ...
        CLR_DP_HDCP14_RX2_AUTH_READ_IRQ_TIMEOUT();
    }
}
...
// Output Value : None
//--------------------------------------------------
bit ScalerDpHdcp14Rx2CheckBinfo(void)
{
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx2LoadKsvFifo(void)
{
    WORD usIndex = 0;
    WORD usKsvOffset = 0;
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx2SetKsvFifoFirst(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);
    ...
    for(g_usDpHdcp14Rx2KsvFifoOffset = 0; g_usDpHdcp14Rx2KsvFifoOffset < 15; g_usDpHdcp14Rx2KsvFifoOffset++)
    {
        if(GET_DP_TX_HDCP_KSV_FIFO_MAX() > g_usDpHdcp14Rx2KsvFifoOffset)
        {
            ...
            ScalerDpAuxRxSetDpcdValue(enumInputPort, 0x06, 0x80, (0x2C + g_usDpHdcp14Rx2KsvFifoOffset), g_pucDpMacTxHdcpKsvFifo[g_usDpHdcp14Rx2KsvFifoOffset]);
        }
        else
        {
            // DPCD [0x6802C ~ 0x6803A]
            ScalerDpAuxRxSetDpcdValue(enumInputPort, 0x06, 0x80, (0x2C + g_usDpHdcp14Rx2KsvFifoOffset), 0x00);
        }
    }
...
//--------------------------------------------------
bit ScalerDpHdcp14Rx2DeviceCountChange(void)
{
    EnumInputPort enumInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);
    BYTE ucBInfoDevice = 0;
    BYTE ucBInfoDepth = 0;
...
//--------------------------------------------------
// Description  : Change DP HDCP1.4 Repeater Capable for Rx2 Base On DPTX Clone Status
// Input Value  : InputPort, PortType, Clone Mode or not
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx2RepeaterSupportProc(void)
{
    bit bDpRx2HdcpSetRepeater = _OFF;
    EnumInputPort enumHdcpInputPort = ScalerDpHdcpRxRxDxMapping(_RX2);

    if(GET_DP_HDCP14_RX2_CAPABLE_SUPPORT() == _FALSE)
    {
        return;
    }
    ...
    if(GET_DP_MAC_TX_DOWNSTREAM_DPPLUG_STATUS() != _DP_UNPLUG)
    {
        if(GET_DP_RX_HDCP_UPSTREAM_PORT() == _HDCP_D2_PORT)
        {
            bDpRx2HdcpSetRepeater = _ON;
        }
    }

    if(bDpRx2HdcpSetRepeater != GET_DP_HDCP14_RX2_REPEATER_SUPPORT())
    {
        DebugMessageHDCP2("[D2]Hdcp1.4 Repeater Bit Changed", bDpRx2HdcpSetRepeater);

        if(bDpRx2HdcpSetRepeater == _ON)
        {
            SET_DP_HDCP14_RX2_REPEATER_SUPPORT();
        }
        else
        {
            CLR_DP_HDCP14_RX2_REPEATER_SUPPORT();
        }
...
// Output Value : None
//--------------------------------------------------
void ScalerDpHdcp14Rx2ReAuthProcess(EnumInputPort enumInputPort)
{
    ...
    if((ScalerDpAuxRxGetDpcdBitInfo(enumInputPort, 0x00, 0x01, 0x11, (_BIT1 | _BIT0)) == 0x00) &&
       (GET_DP_HDCP14_RX2_AUTH_STATE() != _DP_RX_HDCP_STATE_IDLE) &&
       (ScalerDpMacDphyRxHdcpCheckValid(enumInputPort) == _TRUE))
    {
